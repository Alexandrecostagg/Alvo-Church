...
export function BillingGate({ children }: { children: ReactNode }) {
  ...
  if (billingStatus === "suspended" && !ALLOWED_WHEN_SUSPENDED.has(pathname ?? "")) {
    return (
      <div style={{
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
        gap: 16, padding: "4rem 1.5rem", textAlign: "center", minHeight: "60vh",
      }}>
        <div style={{
          ...
          <Lock size={24} style={{ color: "var(--color-text-danger, #A32D2D)" }} />
        </div>
        <div>
          <p style={{ margin: 0, fontWeight: 500, fontSize: 17, color: "var(--color-text-primary, #111)" }}>
            Acesso suspenso por falta de pagamento
          </p>
          <p style={{ margin: "6px 0 0", fontSize: 14, color: "var(--color-text-secondary, #6b7280)", maxWidth: 420 }}>
            Sua organização está com a fatura em atraso há mais de 5 dias. Regularize o pagamento para
            voltar a usar todas as funções da Plataforma Esdras.
          </p>
        </div>
        <a
          href="/settings/plano"
  ...
  return (
    <>
      {billingStatus === "overdue" && <OverdueBanner overdueSince={overdueSince} />}
      {billingStatus === "suspended" && <OverdueBanner overdueSince={overdueSince} suspended />}
      {children}
    </>
  );
}

function OverdueBanner({ overdueSince, suspended }: { overdueSince: string | null; suspended?: boolean }) {
  const since = overdueSince ? new Date(overdueSince) : null;
  const dateLabel = since && !Number.isNaN(since.getTime())
  ...
  return (
    <div style={{
      ...
      <a href="/settings/plano" style={{ color: "inherit", fontWeight: 500, textDecoration: "underline", flexShrink: 0 }}>
        Regularizar agora
      </a>
    </div>
  );
