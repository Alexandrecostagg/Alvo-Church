...
export async function POST(req: NextRequest) {
  ...
  if (!WORKER_API_BASE_URL) {
    return NextResponse.json(
      { error: "WORKER_API_BASE_URL não configurada no servidor." },
      { status: 500 },
    );
  }
  ...
  if (!bearerToken) {
    return NextResponse.json(
      { error: "NOTIFY_API_BEARER_TOKEN não configurado no servidor." },
      { status: 500 },
    );
  }
  ...
  if (!organizationId || !message?.trim() || !recipients?.length) {
    return NextResponse.json(
      { error: "organizationId, message e recipients são obrigatórios." },
      { status: 400 },
    );
  }
  ...
  const allowed = await hasAnyRoleInOrg(
    idToken,
    organizationId,
    uid,
    TENANT_ADMIN_ROLES,
  ...
  if (!allowed) {
    return NextResponse.json(
      {
        error:
          "Você não tem permissão para enviar comunicados desta organização.",
      },
      { status: 403 },
    );
  }
    ...
    recipients.map(async (recipient) => {
      ...
      try {
        const res = await fetch(
          `${WORKER_API_BASE_URL.replace(/\/$/, "")}/notify/whatsapp`,
          {
            method: "POST",
            headers: {
