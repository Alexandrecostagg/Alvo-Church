...
export async function POST(req: NextRequest) {
  try {
    ...
    await verifyTurnstile({ token: body.turnstileToken, expectedAction: "public_visit", remoteIp: clientKey, idempotencyKey: body.turnstileRequestId });
    return NextResponse.json(await capturePublicVisitor(body, clientKey), { headers: { "cache-control": "no-store" } });
  } catch (error) {
