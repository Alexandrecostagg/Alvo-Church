...
export async function POST(req: NextRequest) {
  ...
  let body: { organizationId?: string; planId?: string; orgName?: string; email?: string; cpfCnpj?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (!organizationId || !planId || !orgName || !email) {
    return NextResponse.json({ error: "organizationId, planId, orgName e email são obrigatórios." }, { status: 400 });
  }

  const price = PLAN_PRICE[planId];
  if (!price) {
    return NextResponse.json({ error: `Plano "${planId}" não tem checkout self-service.` }, { status: 400 });
  }
  ...
  try {
    ...
    const subscriptionData = (await subscriptionRes.json()) as { id?: string; errors?: Array<{ description: string }> };
    if (!subscriptionRes.ok || !subscriptionData.id) {
      return NextResponse.json({ error: `Asaas: ${subscriptionData.errors?.[0]?.description ?? "erro ao criar assinatura"}` }, { status: 502 });
    }
    ...
    if (!checkoutUrl) {
      return NextResponse.json({ error: "Assinatura criada, mas não foi possível obter o link de pagamento." }, { status: 502 });
    }
