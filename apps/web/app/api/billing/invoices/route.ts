...
async function isTenantMemberOfOrg(
  idToken: string,
...
async function fetchSubscriptionId(
  ...
  const data = (await res.json()) as {
    fields?: { asaasSubscriptionId?: { stringValue?: string } };
  };
...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(
      `${ASAAS_BASE_URL}/subscriptions/${subscriptionId}/payments?limit=12`,
      {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: "Não foi possível consultar as faturas no Asaas." },
        { status: 502 },
      );
    }
