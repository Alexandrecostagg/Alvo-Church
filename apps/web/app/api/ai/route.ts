...
async function hasPastoralRole(
  idToken: string,
...
async function checkAiQuota(
  ...
  const [subRes, usageRes] = await Promise.all([
    fetch(
      firestoreDocUrl(`organizations/${organizationId}/settings/subscription`),
      {
        headers,
        signal: AbortSignal.timeout(8000),
      },
  ...
  if (subRes.ok) {
    ...
    const explicitPlan = subData.fields?.plan?.stringValue as
      | PlanId
      | undefined;
    plan =
      explicitPlan ??
      planTierToPlanId(subData.fields?.planTier?.stringValue as any);
  }
  ...
  const consume = async () => {
    await fetch(
      `${firestoreDocUrl(`organizations/${organizationId}/aiUsage/${month}`)}?updateMask.fieldPaths=count&updateMask.fieldPaths=updatedAt`,
      {
        method: "PATCH",
        headers,
        signal: AbortSignal.timeout(8000),
...
export async function POST(req: NextRequest) {
  ...
  const [pastoralAllowed, quota] = await Promise.all([
    PASTORAL_ONLY_TASKS.has(task)
      ? hasPastoralRole(idToken, organizationId, uid)
