...
export default function JoinNetworkPage() {
  ...
  async function lookupParent(parentOrgId: string) {
    ...
    try {
      ...
      // Valida o convite
      const affiliate = await fetchNetworkAffiliateByInviteCode(
        config,
        parentOrgId.trim(),
        code,
      );
      ...
      const brandSnap = await getDoc(
        doc(
          db,
          getOrganizationBrandingDocumentPath({
            organizationId: parentOrgId.trim(),
          }),
        ),
      );
      const parentBrand = brandSnap.data() as
        | { publicShortName?: string; publicProductName?: string }
        | undefined;
      setParentName(
        parentBrand?.publicShortName ??
          parentBrand?.publicProductName ??
          parentOrgId.trim(),
      );
      ...
      setErrorMsg(
        "Não foi possível validar o convite. Verifique o código e tente novamente.",
      );
      setStep("error");
    }
  }
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div
            style={{
              ...
              height: 56,
              borderRadius: 14,
              background: "var(--esdras-primary-softer)",
              display: "flex",
              alignItems: "center",
          ...
          >
            <LinkIcon
              size={26}
              style={{ color: "var(--esdras-primary-dark)" }}
            />
          </div>
          <h1
            style={{
        ...
        {step === "enter_org" && (
          <div style={{ display: "grid", gap: 16 }}>
            <div style={notice}>
              ...
              />
              <span>
                Informe o <strong>ID da organização sede</strong> que enviou o
                convite. Este dado está disponível no e-mail ou mensagem
                recebida.
              </span>
            </div>
            ...
            <button
              ...
              style={{
                ...btn,
                opacity: orgId.trim() ? 1 : 0.5,
                cursor: orgId.trim() ? "pointer" : "not-allowed",
              }}
            >
              Validar convite
            </button>
          ...
          <div style={{ display: "grid", gap: 20 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: 12, color: "#64748b" }}>
                    Sede: <strong>{parentName}</strong>
                  </span>
                </div>
              </div>
              {affiliateName && (
                <p style={{ margin: 0, fontSize: 13, color: "#374151" }}>
                  Sua igreja será integrada como{" "}
                  <strong>{affiliateName}</strong> na rede. A sede poderá
                  visualizar apenas dados gerenciais (totais agregados).
                </p>
              )}
            </div>
            ...
            <div style={notice}>
              ...
              <span style={{ fontSize: 12 }}>
                Ao aceitar, a sede visualizará totais como número de membros,
                arrecadação e engajamento. Dados individuais de membros{" "}
                <strong>não são compartilhados</strong>.
              </span>
            </div>
...
const btnSecondary: React.CSSProperties = {
  ...
  padding: "13px",
  borderRadius: 12,
  border: "1px solid rgba(29,41,64,0.18)",
  background: "#fff",
  fontSize: 14,
  fontWeight: 600,
  cursor: "pointer",
