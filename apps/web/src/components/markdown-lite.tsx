...
export function MarkdownLite({ text, style }: { text: string; style?: React.CSSProperties }) {
  ...
  const flushList = () => {
    if (list) {
      const items = list.items.map((it, idx) => (
        <li key={`li-${key}-${idx}`} style={{ marginBottom: 4, lineHeight: 1.6 }}>{renderInline(it, `li${key}${idx}`)}</li>
      ));
      blocks.push(
        list.ordered
          ? <ol key={`ol-${key++}`} style={{ margin: "0 0 12px", paddingLeft: 22 }}>{items}</ol>
          : <ul key={`ul-${key++}`} style={{ margin: "0 0 12px", paddingLeft: 22 }}>{items}</ul>
      );
      list = null;
    }
  };
  ...
  for (const raw of lines) {
    ...
    if (h1 || h2 || h3) {
      ...
      blocks.push(
        <p key={`h-${key++}`} style={{ fontSize: size, fontWeight: 800, margin: "16px 0 8px", color: "var(--alvo-ink, #0f172a)" }}>
      ...
      flushPara(); flushList();
      blocks.push(
        <blockquote key={`q-${key++}`} style={{ margin: "0 0 12px", padding: "8px 14px", borderLeft: "3px solid #534AB7", background: "rgba(83,74,183,0.06)", borderRadius: 6, fontStyle: "italic", lineHeight: 1.6 }}>
  ...
  return <div style={{ fontSize: 14, color: "var(--alvo-ink, #1e293b)", ...style }}>{blocks}</div>;
}
