...
export function GivingView() {
  ...
  const [viewingReceipt, setViewingReceipt] = useState<GivingReceipt | null>(
    null,
  );
  ...
  function broadcastToMembers(c: GivingCampaign) {
    router.push(
      `/communication?compose=${encodeURIComponent(shareMessage(c))}`,
    );
  }

  useEffect(() => {
    if (!isFirebaseWebRuntimeConfigured(firebaseConfig) || !organizationId)
      return;
    fetchGivingIntents(firebaseConfig, { organizationId })
      .then(setIntents)
      .catch(() => {});
    fetchGivingCampaigns(firebaseConfig, { organizationId })
      .then(setCampaigns)
      .catch(() => {});
    fetchGivingReceipts(firebaseConfig, { organizationId })
      .then(setReceipts)
      .catch(() => {});
  }, [organizationId, firebaseConfig]);

  const totalCaptured = useMemo(
    () => intents.reduce((s, i) => s + i.amount, 0),
    [intents],
  );
  ...
  async function handleCreateCampaign() {
    if (
      ...
      !user ||
      !form.title.trim() ||
      parseAmount(form.goal) <= 0
    )
      return;
    setSaving(true);
    try {
      const id = await saveGivingCampaign(
        firebaseConfig,
        { organizationId },
        {
  ...
  function handleRegisterRaise(c: GivingCampaign) {
    const raw = window.prompt(
      `Quanto entrou nesta campanha? (some ao total atual de ${formatBRL(c.raisedAmount)})`,
      "",
    );
    if (!raw) return;
    ...
    <div className="page-root">
      <header className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">
            Dízimos, ofertas e campanhas da organização
          </p>
        </div>
      ...
      {showForm && (
        <section className="content-section">
          ...
          <div style={{ display: "grid", gap: 10, maxWidth: 560 }}>
            <input
              type="text"
              placeholder="Título (ex: Reforma do templo)"
              value={form.title}
              onChange={(e) =>
            ...
            />
            <textarea
              placeholder="Descrição da causa (opcional)"
              rows={2}
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <input
                type="text"
                placeholder="Categoria (ex: Obras, Missões)"
                value={form.category}
                onChange={(e) =>
            ...
            <button
              ...
              disabled={
                saving || !form.title.trim() || parseAmount(form.goal) <= 0
              }
              style={{
                justifySelf: "start",
                opacity:
        ...
        {campaigns.length === 0 ? (
          <div className="empty-state">
            ...
            <p className="empty-hint">
              Crie uma campanha para arrecadar para um projeto ou causa
              específica.
            </p>
            <button
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {campaigns.map((c) => {
              ...
              const pct =
                c.goalAmount > 0
                  ? Math.min(100, Math.round((raised / c.goalAmount) * 100))
                  : 0;
              return (
                <div
                  key={c.id}
                ...
                >
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        <strong
                          style={{ fontSize: 16, color: "var(--alvo-ink)" }}
                        >
                          {c.title}
                        </strong>
                        {c.category && (
                          <span
                            style={{
                              ...
                              padding: "2px 8px",
                              borderRadius: 999,
                              background: "var(--alvo-accent-soft, #eff6ff)",
                              color: "var(--alvo-accent-dark, #1e3a8a)",
                            }}
                          >
                            {c.category}
                          </span>
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    {auto > 0 && (
                      ...
                      >
                        {formatBRL(auto)} pelo link público ·{" "}
                        {formatBRL(c.raisedAmount)} registrado manualmente
                      </p>
                    )}
                  </div>
                  ...
                  >
                    {c.status === "active" && (
                      <>
                        ...
                        <button
                          className="btn-secondary btn-sm"
                          onClick={() =>
                            updateCampaign(c, { status: "closed" })
                          }
                        >
                          Encerrar
                        </button>
                    ...
                    {c.status === "closed" && (
                      <button
                        className="btn-secondary btn-sm"
                        onClick={() => updateCampaign(c, { status: "active" })}
                      >
                        Reabrir
                      </button>
                    )}
      ...
      <section className="content-section">
        <div className="section-header">
          ...
          {intents.length > 0 && (
            <span
              style={{ fontSize: 13, color: "var(--alvo-ink-soft, #64748b)" }}
            >
              {formatBRL(totalCaptured)} em intenções
            </span>
          )}
        </div>
        {intents.length === 0 ? (
          <div className="empty-state">
            ...
            <p className="empty-hint">
              As doações feitas pelo link público (/p/&lt;sua-igreja&gt;/give)
              aparecerão aqui, com o contato do doador.
            </p>
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 12,
                    background: "var(--alvo-surface, #fff)",
                    border: "1px solid var(--alvo-line, #e2e8f0)",
                  }}
                >
                  <div style={{ minWidth: 0 }}>
                    ...
                    <span
                      style={{
                        fontSize: 12,
                        color: "var(--alvo-ink-soft, #64748b)",
                      }}
                    >
