...
function buildNotifications(
  ...
  const newVisitors = people
    .filter((p) => {
      const j = joinedAt(p);
      return p.memberStatus === "visitor" && j !== undefined && j >= monthAgo;
    })
  ...
  const overdueTasks = tasks
    .filter(
      (t) =>
        t.status === "open" &&
        t.dueAt &&
        t.dueAt < now.toISOString().slice(0, 10),
    )
    .slice(0, 3);
  ...
  const upcoming = events
    .filter(
      (e) =>
        e.startsAt &&
        e.startsAt.slice(0, 10) >= now.toISOString().slice(0, 10) &&
        e.startsAt.slice(0, 10) <= in3days,
    )
    .slice(0, 2);
        ...
        {(["all", "unread"] as const).map((f) => (
          <button
            ...
            style={{
              ...
              border: "1.5px solid",
              borderColor:
                filter === f ? "var(--alvo-accent-dark)" : "var(--alvo-line)",
              background:
                filter === f ? "var(--alvo-accent-soft)" : "transparent",
              color:
                filter === f
      ...
      >
        {visible.length === 0 ? (
          <div style={{ padding: "48px 24px", textAlign: "center" }}>
            <Bell
              size={32}
              style={{ color: "var(--alvo-line)", marginBottom: 12 }}
            />
            <p
              style={{ color: "var(--alvo-ink-soft)", fontSize: 14, margin: 0 }}
            >
              {filter === "unread"
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    >
                      <span
                        style={{ fontSize: 11, color: "var(--alvo-ink-soft)" }}
                      >
                        {n.time}
                      </span>
                      {!n.read && (
