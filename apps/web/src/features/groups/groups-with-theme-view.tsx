  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 4, padding: "16px 20px 0" }}>
        <TabButton active={tab === "groups"} onClick={() => setTab("groups")} icon={Waypoints} label="Células" />
        <TabButton active={tab === "theme"} onClick={() => setTab("theme")} icon={BookOpen} label="Tema Semanal" />
      </div>
