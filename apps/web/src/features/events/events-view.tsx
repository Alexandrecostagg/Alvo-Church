...
import type {
  ...
  EventRegistration as DomainEventRegistration,
} from "@alvo/types";
...
const initialEvents: EventType[] = [
  {
    ...
    description:
      "O maior encontro de mulheres do Plataforma Esdras. Três dias de imersão espiritual profunda na identidade, propósito e cura emocional para abençoar sua vida.",
    type: "conference",
    status: "published",
    locationType: "onsite",
  ...
  },
  {
    ...
    description:
      "A pública profissão de fé e celebração da ressurreição espiritual de dezenas de novos discípulos que descerão às águas no Plataforma Esdras.",
    type: "service",
    status: "published",
    locationType: "onsite",
  ...
  },
  {
    ...
    description:
      "Treinamento intensivo de sobrevivência, imersão em dinâmicas de tribo e alinhamento da liderança de células para a grande colheita de 2026.",
    type: "camp",
    status: "published",
    locationType: "onsite",
...
const mockScheduleMap: Record<string, VolunteerAssignment[]> = {
  event_women_2026: [
    {
      ...
      volunteerName: "Ana Silva",
      status: "confirmed",
      teamName: "Acolhimento",
    },
    {
      id: "v2",
      role: "Operador de Som",
      volunteerName: "Felipe Andrade",
      status: "confirmed",
      teamName: "Mídia/Som",
    },
    {
      id: "v3",
      role: "Câmera 1",
      volunteerName: "Rodrigo Melo",
      status: "pending",
      teamName: "Mídia/Som",
    },
    {
      id: "v4",
      role: "Apoio Diaconato",
      volunteerName: "Matheus Costa",
      status: "confirmed",
      teamName: "Ordem e Protocolo",
    },
    {
      id: "v5",
      role: "Recepcionista Portal",
      volunteerName: "Clara Santos",
      status: "pending",
      teamName: "Acolhimento",
    },
  ],
  ...
  event_leadership_camp: [
    ...
    },
    {
      id: "v22",
      role: "Socorrista",
      volunteerName: "Dra. Patrícia Lima",
      status: "confirmed",
      teamName: "Saúde",
    },
    {
      id: "v23",
      role: "Logística Sítio",
      volunteerName: "Carlos Silveira",
      status: "pending",
...
const mockWorshipMap: Record<string, WorshipSong[]> = {
  event_women_2026: [
    {
      ...
      artist: "Esdras Worship",
      key: "D",
      links: {
        chords: "https://cifraclub.com.br",
  ...
  event_leadership_camp: [
    {
      id: "s20",
      title: "Tua Presença é o Meu Bem",
      artist: "Esdras Worship",
      key: "E",
      links: {
        chords: "https://cifraclub.com.br",
...
const TYPE_FROM_DOMAIN: Record<DomainEvent["type"], EventType["type"]> = {
  ...
  retreat: "camp",
  training: "training",
  integration_class: "training",
  kids_event: "celebration",
};
...
function regToAttendee(reg: DomainEventRegistration): Attendee {
  const name =
    (reg.personName && reg.personName.trim()) || reg.registrationCode;
  ...
  return {
    ...
    checkedInAt: reg.checkedInAt
      ? new Date(reg.checkedInAt).toLocaleTimeString("pt-BR", {
          hour: "2-digit",
          minute: "2-digit",
        })
      : undefined,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setErr(
          "Não foi possível acessar a câmera. Permita o acesso no navegador ou use a marcação manual abaixo.",
        );
      }
    })();
    return () => {
...
export function EventsView() {
  const { configured, firebaseReady, firebaseConfig, organizationId } =
    useAppAuth();
  ...
  const [typeFilter, setTypeFilter] = useState<
    "all" | "conference" | "service" | "camp" | "training" | "celebration"
  >("all");

  // Novos estados para Abas e Calendário
  const [activeTab, setActiveTab] = useState<
    "attendees" | "schedule" | "worship"
  >("attendees");
  const [sidebarMode, setSidebarMode] = useState<"list" | "calendar">("list");
  const [notifiedAttendeeId, setNotifiedAttendeeId] = useState<string | null>(
    null,
  );
  ...
  const [regsById, setRegsById] = useState<
    Record<string, DomainEventRegistration>
  >({});
  const [worshipLib, setWorshipLib] = useState<
    Record<
      string,
      {
        title: string;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const domain = await fetchEvents(
          firebaseConfig,
          { organizationId },
          100,
        );
        ...
        try {
          const regs = await fetchEventRegistrations(
            firebaseConfig,
            { organizationId },
            domain,
            500,
  ...
  useEffect(() => {
    let cancelled = false;
    if (!configured || !firebaseReady || !organizationId || !selectedEventId) {
      setWorshipForActive([]);
      return;
    }
    (async () => {
      try {
        const setlist = await fetchWorshipSetlistByEventId(
          firebaseConfig,
          { organizationId },
          selectedEventId,
        );
        ...
        const songs: WorshipSong[] = [...setlist.songs]
          ...
          .map((s) => {
            ...
            return {
              ...
              links: {
                chords: undefined,
                youtube: lib?.youtubeUrl,
                spotify: lib?.spotifyUrl,
              },
            };
          });
  ...
  const [selectedScanAttendeeId, setSelectedScanAttendeeId] =
    useState<string>("");
  ...
  const [inspectedAttendee, setInspectedAttendee] = useState<Attendee | null>(
    null,
  );
  ...
  const stats = useMemo(() => {
    ...
    const paid = activeAttendees.filter(
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      await saveEvent(
        firebaseConfig,
        { organizationId },
        viewToDomain(savedEvent, organizationId),
      );
    } catch (err) {
      console.error("saveEvent falhou:", err);
      setNotificationBanner({
        message: "Não foi possível salvar o evento. Tente de novo.",
        type: "info",
      });
      return;
    }
    ...
    setNotificationBanner({
      message: isEdit
        ? `Evento "${savedEvent.name}" atualizado.`
        : `Evento "${savedEvent.name}" publicado — já aparece no app.`,
      type: "success",
    });
    ...
    try {
      ...
      setEvents((prev) => {
        ...
        setSelectedEventId((cur) =>
          cur === evt.id ? (next[0]?.id ?? "") : cur,
        );
        return next;
      });
      setNotificationBanner({
        message: `Evento "${evt.name}" excluído.`,
        type: "info",
      });
    } catch (err) {
  ...
  const handleAddGuest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotificationBanner({
        message: "Não foi possível registrar o convidado. Tente de novo.",
        type: "info",
      });
      return;
    }
  ...
  const persistCheckIn = async (
    ...
    if (!reg)
      return {
        success: false,
        message: "Ingresso não encontrado ou de outro evento.",
      };
    if (reg.checkedInAt) {
      const at = new Date(reg.checkedInAt).toLocaleTimeString("pt-BR", {
        hour: "2-digit",
        minute: "2-digit",
      });
      return {
        success: false,
        message: `Ingresso ${reg.registrationCode} já utilizado às ${at}.`,
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        message: "Não foi possível registrar o check-in. Tente de novo.",
      };
    }
    ...
    setAttendeesMap((prev) => {
      ...
      return {
        ...prev,
        [reg.eventId]: list.map((a) =>
          a.id === attendeeId
            ? {
                ...a,
                checkedIn: true,
                checkedInAt: new Date(nowIso).toLocaleTimeString("pt-BR", {
                  hour: "2-digit",
                  minute: "2-digit",
                }),
              }
    ...
    return {
      success: true,
      message: `Check-in confirmado para ${nome}! Seja bem-vindo!`,
    };
  };

  // Check-in manual rápido na lista.
  const handleQuickCheckin = (attendeeId: string) => {
    void persistCheckIn(attendeeId);
  };
  ...
  const confirmPayment = async (attendeeId: string) => {
    ...
    const updated: DomainEventRegistration = {
      ...reg,
      paymentStatus: "paid",
    ...
    setAttendeesMap((prev) => {
      ...
      return {
        ...prev,
        [reg.eventId]: list.map((a) =>
          a.id === attendeeId ? { ...a, paymentStatus: "paid" as const } : a,
        ),
      };
    });
    setNotificationBanner({
      message: `Pagamento de ${(reg.personName && reg.personName.trim()) || reg.registrationCode} confirmado.`,
      type: "success",
    });
  };
  ...
  return (
    ...
    >
          ...
          >
            ...
            <div
              style={{
                ...
                border: scanning
                  ? "2px solid var(--alvo-accent)"
                  : "2px dashed var(--alvo-line)",
                position: "relative",
                overflow: "hidden",
                display: "flex",
            ...
            >
              ...
              {scanResult ? (
                ...
                <QrCameraScanner
                  active={showScanner && !scanResult}
                  onDetect={(txt) => void handleScanCode(txt)}
                />
              )}
            </div>
            ...
            >
              ...
              >
                Ou marque a presença manualmente:
              </label>
              <select
              ...
              >
                <option value="" style={{ color: "var(--alvo-ink)" }}>
                  -- Escolha um inscrito --
                </option>
                {activeAttendees.map((att) => (
                  <option
                    key={att.id}
                    value={att.id}
                    style={{ color: "var(--alvo-ink)" }}
                  >
                    {att.firstName} {att.lastName} (
                    {att.checkedIn ? "✓ Já Fez" : "⏳ Pendente"})
                  </option>
                ))}
              </select>
          ...
          >
            ...
            <div
              style={{
                ...
                padding: "2rem 1.5rem",
                background:
                  "linear-gradient(135deg, rgba(255, 255, 255, 0.9) 0%, rgba(243, 244, 246, 0.7) 100%)",
                position: "relative",
                overflow: "hidden",
                marginTop: "1.25rem",
              ...
              <div>
                ...
                >
                  <div>
                    ...
                    <strong
                      style={{ color: "var(--alvo-ink)", fontWeight: 700 }}
                    >
                  ...
                  <div>
                    ...
                    <strong
                      style={{
                        color: inspectedAttendee.checkedIn
                          ? "var(--alvo-green)"
                          : "var(--alvo-accent)",
                      }}
                    >
                      {inspectedAttendee.checkedIn
                        ? `✓ Confirmado às ${inspectedAttendee.checkedInAt}`
                        : "⏳ Pendente"}
                    </strong>
                  </div>
                </div>
                ...
                {activeEvent.isPaid && (
                  ...
                  >
                    ...
                    {inspectedAttendee.paymentStatus !== "paid" && (
                      ...
                      >
                        ...
                        >
                          ...
                          . O membro paga o <strong>PIX real da igreja</strong>{" "}
                          pelo app. Confira o comprovante e confirme quando o
                          valor cair na conta.
                        </p>
                        {regsById[inspectedAttendee.id]?.receiptImage ? (
                          <img
                            src={
                              regsById[
                                inspectedAttendee.id
                              ]!.receiptImage!.startsWith("data:")
                                ? regsById[inspectedAttendee.id]!.receiptImage!
                                : `data:image/jpeg;base64,${regsById[inspectedAttendee.id]!.receiptImage}`
                            }
                            alt="Comprovante PIX"
                            style={{
                              width: "100%",
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            void confirmPayment(inspectedAttendee.id);
                    ...
                    >
                      ...
                      >
                        A cobrança está ativa na área pessoal do membro e pode
                        ser paga diretamente via aplicativo.
                      </p>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setTimeout(() => {
                            alert(
                              `A cobrança já está visível para ${inspectedAttendee.firstName} no aplicativo. O reenvio de notificação push estará disponível em breve.`,
                            );
                          }, 100);
                        }}
                        style={{
                          ...
                          padding: "0.45rem",
                          backgroundColor:
                            notifiedAttendeeId === inspectedAttendee.id
                              ? "var(--alvo-green)"
                              : "var(--alvo-accent)",
                          color: "white",
                          border: "none",
                      ...
                      >
                        {notifiedAttendeeId === inspectedAttendee.id
                          ? "✓ Cobrança Notificada no Celular!"
                          : "🔔 Reenviar Notificação no Celular"}
                      </button>
                    </div>
                  </div>
                )}
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {editingId ? "EDIÇÃO" : "NOVO COMPROMISSO"}
                </span>
                <h3
                  style={{
                    fontSize: "1.5rem",
                    ...
                    onChange={(e) =>
                      setNewEvent((prev) => ({
                        ...
                        locationType: e.target
                          .value as EventType["locationType"],
                      }))
                    }
                    style={{
                      width: "100%",
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      Evento Pago (Venda de Ingressos)
                    </strong>
                    <span
                      style={{
                        fontSize: "0.75rem",
                    ...
                    >
                      Habilitar cobrança de entrada para custos
                    </span>
                  </div>
                  <input
                    type="checkbox"
                ...
                {newEvent.isPaid && (
                  ...
                  >
                    ...
                    <input
                      ...
                      placeholder="R$ 0,00"
                      value={
                        newEvent.ticketPrice !== undefined
                          ? newEvent.ticketPrice.toLocaleString("pt-BR", {
                              style: "currency",
                              currency: "BRL",
                            })
                          : ""
                      ...
                      onChange={(e) => {
                        ...
                        const numericVal = cleanVal
                          ? parseFloat(cleanVal) / 100
                          : undefined;
                        setNewEvent((prev) => ({
                          ...prev,
                          ticketPrice: numericVal,
                        }));
                      }}
                      style={{
                        width: "100%",
      ...
      <header className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">
            Coordene retiros, conferências e cultos unificados com check-in
            automatizado.
          </p>
        </div>
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          <div className="stat-body">
            ...
            <span className="stat-value">
              {Object.values(attendeesMap).reduce(
        ...
        <div className="stat-card">
          <div className="stat-body">
            ...
            <span className="stat-value">
              {(() => {
                const total = Object.entries(attendeesMap).reduce(
                  (sum, [eid, atts]) => {
                    const price =
                      events.find((e) => e.id === eid)?.ticketPrice ?? 0;
                    return (
                      sum +
                      atts.filter((a) => a.paymentStatus === "paid").length *
                        price
                    );
                  },
                  0,
          ...
          <div className="stat-body">
            ...
            <span className="stat-value">
              {(() => {
                ...
                return all.length
                  ? Math.round(
                      (all.filter((a) => a.checkedIn).length / all.length) *
                        100,
                    ) + "%"
                  : "—";
              })()}
      ...
      <section className="events-grid">
        ...
        <aside className="events-sidebar panel" style={{ padding: "1.5rem" }}>
          ...
          {sidebarMode === "list" ? (
                    ...
                    style={{
                      ...
                      background:
                        typeFilter === cat
                          ...
                          : "rgba(15, 23, 42, 0.04)",
                      color:
                        typeFilter === cat ? "white" : "var(--alvo-ink-soft)",
                      transition: "var(--alvo-transition-creamy)",
                      boxShadow:
                        typeFilter === cat
                    ...
                    return (
                      <div
                        ...
                        style={{
                          background: isSelected
                            ? "linear-gradient(135deg, rgba(37, 99, 235, 0.06) 0%, rgba(37, 99, 235, 0.01) 100%)"
                            : "var(--alvo-surface)",
                      ...
                      >
                        ...
                        >
                          <strong
                            style={{
                              color: isSelected ? "white" : "var(--alvo-ink)",
                            }}
                          >
                            {date.getDate()}
                          </strong>
            ...
            <div
              className="mini-calendar-container animate-entrance"
        ...
        <article
          className="event-detail-panel panel"
          style={{ boxShadow: "var(--alvo-shadow-airy)" }}
        >
          {activeEvent ? (
            <>
              {/* Top Details Header */}
              <div className="detail-header">
                <div className="title-area" style={{ flex: 1 }}>
                  <span
                    className={`event-type-badge ${activeEvent.type}`}
                    style={{
                      background: "var(--alvo-accent-soft)",
                      color: "var(--alvo-accent)",
                    }}
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      {activeEvent.isPaid
                        ? `R$ ${(activeEvent.ticketPrice ?? 0).toFixed(2).replace(".", ",")}`
                        : "Gratuito"}
                    </span>
                    <span
                      style={{
                    ...
                    >
                      ...
                      />
                      {new Date(activeEvent.startsAt).toLocaleString("pt-BR", {
                        dateStyle: "short",
                        timeStyle: "short",
                      })}
                    </span>
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      if (
                        window.confirm(
                          `Excluir o evento "${activeEvent.name}"? Some do web e do app.`,
                        )
                      )
                        void handleDeleteEvent(activeEvent);
                    }}
                    ...
                    style={{
                      ...
                      fontWeight: 700,
                      cursor: "pointer",
                      opacity: deletingId === activeEvent.id ? 0.6 : 1,
                    }}
                  >
                    <X size={16} /> Excluir
                  </button>
              ...
              >
                ...
                >
                  ...
                  <div>
                    <strong
                      style={{ color: "var(--alvo-ink)", fontWeight: 800 }}
                    >
                      {stats.total} / {activeEvent.capacity || "∞"}
                    </strong>
                    <span
                      style={{ color: "var(--alvo-ink-soft)", fontWeight: 600 }}
                    >
                      Inscritos Totais
                    </span>
                  </div>
              ...
              {activeTab === "attendees" && (
                <div
                  className="registrations-section animate-entrance"
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        Gestão de presenças e credenciamento de ingressos
                      </p>
                    </div>
                    ...
                    {filteredAttendees.length === 0 ? (
                      ...
                      >
                        ...
                        <p style={{ fontSize: "0.9rem", fontWeight: 500 }}>
                          Nenhum convidado inscrito ou localizado na busca.
                        </p>
                      </div>
                    ) : (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                Inscrito em {att.registrationDate} ·{" "}
              ...
              {activeTab === "schedule" && (
                <div
                  className="schedule-section animate-entrance"
                  style={{
                    display: "flex",
                    flexDirection: "column",
                    gap: "1.25rem",
                ...
                >
                  <div>
                    ...
                    >
                      Voluntários escalados para este evento e status de
                      confirmação
                    </p>
                  </div>
                  ...
                  <div style={{ display: "grid", gap: "0.75rem" }}>
                    {scheduleForActive.length === 0 ? (
                      ...
                      >
                        ...
                        />
                        <p style={{ fontSize: "0.9rem", fontWeight: 500 }}>
                          Nenhum voluntário escalado para a data deste evento.
                        </p>
                        <p style={{ fontSize: "0.75rem", marginTop: 4 }}>
                          As escalas são geridas no módulo Escalas (por data de
                          serviço).
                        </p>
                      </div>
                    ) : (
                        ...
                        >
                          ...
                          >
                            ...
                            <div>
                              ...
                              >
                                {assignment.role} ·{" "}
                                <code style={{ color: "var(--alvo-accent)" }}>
                                  {assignment.teamName}
                                </code>
                              </span>
                            </div>
              ...
              {activeTab === "worship" && (
                <div
                  className="worship-section animate-entrance"
                  style={{
                    display: "flex",
                    flexDirection: "column",
                    gap: "1.25rem",
                ...
                >
                  <div>
                    ...
                    >
                      Músicas selecionadas para o culto/evento e materiais de
                      ensaio
                    </p>
                  </div>
                  ...
                  <div style={{ display: "grid", gap: "0.75rem" }}>
                    {worshipForActive.length === 0 ? (
                      ...
                      >
                        ...
                        />
                        <p style={{ fontSize: "0.9rem", fontWeight: 500 }}>
                          Nenhuma música no setlist deste evento.
                        </p>
                        <p style={{ fontSize: "0.75rem", marginTop: 4 }}>
                          Monte o repertório no módulo Louvor/Worship.
                        </p>
                      </div>
                    ) : (
            ...
            <div className="empty-selection">
              ...
              />
              <p style={{ color: "var(--alvo-ink-soft)", fontWeight: 600 }}>
      ...
      <style jsx>{`
        ...
        .reg-item:hover {
          background: var(--alvo-surface) !important;
          border-color: rgba(37, 99, 235, 0.3) !important;
          transform: translateY(-1.5px);
          box-shadow: 0 6px 15px rgba(15, 23, 42, 0.06) !important;
        }
        .reg-person strong {
          display: block;
        }
