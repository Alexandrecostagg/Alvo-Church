...
export function ReportsView() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        /* mantém vazio — mostra estado honesto */
      } finally {
        setLoading(false);
      }
    }
  ...
  const newThisMonth = people.filter((p) => {
    const j = joinedAt(p);
    return j !== undefined && j >= monthStartISO;
  }).length;
  ...
  const givingPrevMonth =
    incomeByMonth.get(monthKey(new Date(y, m - 1, 1))) ?? 0;
  const hasFinance = txns.length > 0;

  // Série real dos últimos 6 meses.
  const givingSeries = Array.from(
    { length: 6 },
    (_, i) => incomeByMonth.get(monthKey(new Date(y, m - 5 + i, 1))) ?? 0,
  );
  const givingLabels = Array.from(
    { length: 6 },
    (_, i) => MONTHS[new Date(y, m - 5 + i, 1).getMonth()],
  );
  const givingDelta = givingPrevMonth
    ? ((givingThisMonth - givingPrevMonth) / givingPrevMonth) * 100
    : 0;
  ...
  const kpis: KpiBlock[] = [
    {
      label: "Membros",
      value: totalMembers,
      sub: newThisMonth ? `+${newThisMonth} este mês` : "total atual",
      color: "#2563eb",
      bg: "#eff6ff",
    },
    {
    ...
    {
      label: "Arrecadação",
      value: hasFinance ? fmtBRL(givingThisMonth) : "—",
      sub: "mês atual",
      color: "#16a34a",
      bg: "#f0fdf4",
    },
  ...
  for (const p of people) {
    ...
    if (p.tribePrimaryCode)
      tribeCounts.set(
        p.tribePrimaryCode,
        (tribeCounts.get(p.tribePrimaryCode) ?? 0) + 1,
      );
    else semTribo++;
  }
  const tribeData = [...tribeCounts.entries()]
    .map(([code, value], i) => ({
      label: getTribeDisplayLabel(code),
      value,
      color: TRIBE_PALETTE[i % TRIBE_PALETTE.length]!,
    }))
    .sort((a, b) => b.value - a.value);
  if (semTribo)
    tribeData.push({ label: "Sem tribo", value: semTribo, color: "#94a3b8" });
  ...
  function exportCSV() {
    ...
    if (hasFinance)
      rows.push(["Arrecadação", fmtBRL(givingThisMonth), "Mês atual"]);
    ...
    <div className="page-root" ref={printRef}>
      ...
      <header className="page-header no-print">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">
            Visão gerencial consolidada · gerado em {generatedAt}
          </p>
        </div>
          ...
          <div key={k.label} className="stat-card">
            ...
            >
              {k.label === "Membros" ? (
                ...
                <UserPlus size={18} />
              ) : k.label === "Grupos ativos" ? (
                <Waypoints size={18} />
              ) : k.label === "Eventos" ? (
                <CalendarRange size={18} />
              ) : k.label === "Novos membros" ? (
                <TrendingUp size={18} />
              ) : (
                <Heart size={18} />
              )}
            ...
            <div className="stat-body">
              ...
              <span
                ...
                style={{
                  fontSize:
                    typeof k.value === "string" && k.value.length > 6
                      ? "1.1rem"
                      : undefined,
                }}
              >
              ...
              {k.sub && (
                <span style={{ fontSize: 11, color: "var(--alvo-ink-soft)" }}>
                  {k.sub}
                </span>
              )}
            </div>
          ...
          {hasFinance && (
            <span
              style={{
                ...
                padding: "2px 8px",
                borderRadius: 6,
                background: givingDelta >= 0 ? "#dcfce7" : "#fee2e2",
                color: givingDelta >= 0 ? "#16a34a" : "#dc2626",
              }}
            >
              {givingDelta >= 0 ? "+" : ""}
              {givingDelta.toFixed(1)}% vs. mês anterior
            </span>
          )}
        </div>
        {hasFinance ? (
          <>
            ...
            >
              <span style={{ fontSize: 10, color: "var(--alvo-ink-soft)" }}>
                {givingLabels[0]}
              </span>
              <span style={{ fontSize: 10, color: "var(--alvo-ink-soft)" }}>
                {givingLabels[5]}
          ...
          <p style={{ color: "var(--alvo-ink-soft)", fontSize: 13, margin: 0 }}>
            Nenhum lançamento financeiro registrado ainda. Os valores aparecem
            aqui conforme entradas são lançadas em Finanças.
          </p>
        )}
      </section>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <section className="content-section">
          ...
          {totalPeople === 0 ? (
            <p
              style={{ color: "var(--alvo-ink-soft)", fontSize: 13, margin: 0 }}
            >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {statusCounts.map((s) => {
                const pct = totalPeople
                  ? Math.round((s.value / totalPeople) * 100)
                  : 0;
                return (
                  <div
                    key={s.label}
        ...
        <section className="content-section">
          ...
          {tribeData.length === 0 ? (
            <p
              style={{ color: "var(--alvo-ink-soft)", fontSize: 13, margin: 0 }}
            >
              Nenhum membro classificado em tribos ainda.
            </p>
          ) : (
      ...
      <section className="content-section">
        ...
        >
          <thead>
            <tr style={{ borderBottom: "2px solid var(--alvo-line)" }}>
              {["Indicador", "Mês Atual", "Mês Anterior", "Variação"].map(
                (h) => (
                  <th
                    key={h}
                    style={{
          ...
          <tbody>
            {[
              {
                ind: "Total de Membros",
                cur: String(totalMembers),
                prev: null,
                delta: null,
              },
              {
                ind: "Novos Membros (mês)",
                cur: String(newThisMonth),
                prev: null,
                delta: null,
              },
              {
                ind: "Visitantes",
                cur: String(totalVisitors),
                prev: null,
                delta: null,
              },
              {
                ind: "Grupos Ativos",
                cur: String(activeGroups),
                prev: null,
                delta: null,
              },
              {
                ind: "Eventos",
                cur: String(totalEvents),
                prev: null,
                delta: null,
              },
              ...(hasFinance
              ...
              return (
                <tr
                  ...
                  style={{
                    ...
                    background:
                      i % 2 === 0 ? "transparent" : "var(--alvo-surface-muted)",
                  }}
                >
                  <td
                    style={{
        ...
        >
          Comparativos mês a mês disponíveis para indicadores com histórico
          registrado (ex.: arrecadação). Demais indicadores refletem o estado
          atual.
        </p>
      </section>
