...
const S = {
  ...
  divider: { borderBottom: "1px solid var(--alvo-border)" } as React.CSSProperties,
...
export function MemberProfileView() {
  ...
  return (
    <main className="form-page profile-page animate-entrance" style={{ maxWidth: 1400, padding: "2rem" }}>
      ...
      {/* Cabeçalho do Perfil */}
      <section className="profile-hero" style={{ borderBottom: "1px solid var(--alvo-border)", paddingBottom: "2rem" }}>
        <div>
          <Link className="back-link" href="/members" style={{ color: "var(--getro-primary)" }}>
            Voltar para membros
          </Link>
          <p className="eyebrow" style={{ color: "var(--getro-primary)" }}>Ficha Inteligente</p>
          <h1>{fullName}</h1>
        ...
        <aside style={{ backgroundColor: "var(--alvo-surface)", border: "1px solid var(--alvo-border)", borderRadius: 24, padding: "1.5rem" }}>
          <span style={{ fontSize: "0.8rem", color: "var(--getro-primary)", textTransform: "uppercase", fontWeight: 800 }}>
      ...
      {person ? (
        <>
          ...
          <section className="profile-summary-grid" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "1.25rem", marginTop: "2rem" }}>
            ...
            <article style={S.card}>
              <span style={S.label}>Parcerias / Getro Pass</span>
            ...
            <article style={{ ...S.card, backgroundColor: "var(--getro-primary-softer)", borderLeft: "4px solid var(--getro-primary)" }}>
              <span style={{ fontSize: "0.75rem", color: "var(--getro-primary)" }}>Tribo de Dons</span>
              <strong style={S.value}>
                {person.tribePrimaryCode ? getTribeDisplayLabel(person.tribePrimaryCode) : "Asher"}
              </strong>
              <p style={{ ...S.hint, color: "var(--getro-primary)" }}>
          ...
          <section style={{ marginTop: "2.5rem" }}>
            <div style={S.cardLg}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
                <div>
                  <span style={{ fontSize: "0.8rem", color: "var(--getro-primary)", textTransform: "uppercase", fontWeight: 800 }}>Funil de Integração</span>
                  <h2 style={{ fontSize: "1.5rem", fontWeight: 900, color: "var(--alvo-ink)", marginTop: 4 }}>Linha do Tempo da Jornada Pastoral</h2>
                </div>
                <span style={{ fontSize: "0.8rem", background: "var(--getro-primary-soft)", color: "var(--getro-primary)", padding: "6px 12px", borderRadius: 10, fontWeight: 700 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", position: "relative", padding: "1rem 0" }}>
                <div style={{ position: "absolute", top: "50%", left: 0, right: 0, height: 4, backgroundColor: "var(--alvo-border)", zIndex: 1, transform: "translateY(-50%)" }} />
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 36,
                          borderRadius: "50%",
                          backgroundColor: isActive ? "var(--getro-primary)" : isCompleted ? "var(--getro-primary-dark)" : "var(--alvo-surface-muted)",
                          border: isActive ? "4px solid var(--getro-primary-soft)" : "2px solid var(--alvo-border)",
                          display: "flex",
                          alignItems: "center",
                      ...
                      <strong style={{ color: isActive ? "var(--getro-primary)" : isCompleted ? "var(--alvo-ink)" : "var(--alvo-ink-soft)", fontSize: "0.85rem", marginTop: 12, display: "block" }}>
                        {stage.label}
                      </strong>
          ...
          <section className="profile-grid" style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "2rem", marginTop: "2.5rem" }}>
            ...
            <article style={S.cardLg}>
              ...
              <div style={{ display: "flex", borderBottom: "1px solid var(--alvo-border)", paddingBottom: "1rem", gap: "1.5rem" }}>
                {([
                  ...
                  <button
                    ...
                    style={{
                      background: "none",
                      border: "none",
                      color: activeTab === tab.key ? "var(--getro-primary)" : "var(--alvo-ink-soft)",
                      fontWeight: 800,
                      fontSize: "0.95rem",
                      cursor: "pointer",
                      display: "flex",
                      ...
                      gap: 8,
                      paddingBottom: "12px",
                      borderBottom: activeTab === tab.key ? "3px solid var(--getro-primary)" : "none"
                    }}
                  >
                    {tab.icon}
                    {tab.label}
              ...
              <div style={{ marginTop: "2rem" }}>
                ...
                {activeTab === "cell" && (
                  <div className="animate-entrance">
                    ...
                    <div style={{ marginTop: "2rem" }}>
                      <h4 style={{ fontSize: "0.9rem", color: "var(--getro-primary)", fontWeight: 800, marginBottom: "1rem" }}>Rastreador de Presenças (Últimas 4 reuniões):</h4>
                ...
                {activeTab === "finance" && (
                  <div className="animate-entrance">
                      ...
                      <form onSubmit={handleAddContribution} style={S.cardMuted}>
                        ...
                        <div style={{ display: "flex", gap: "0.5rem" }}>
                          <select
                            ...
                            style={{ flex: 1, padding: "8px", backgroundColor: "var(--alvo-surface)", border: "1px solid var(--alvo-border)", borderRadius: 8, color: "var(--alvo-ink)" }}
                          >
                          ...
                          <input
                            ...
                            style={{ flex: 1, padding: "8px", backgroundColor: "var(--alvo-surface)", border: "1px solid var(--alvo-border)", borderRadius: 8, color: "var(--alvo-ink)" }}
                          />
                          <button type="submit" style={{ backgroundColor: "var(--getro-primary)", color: "white", border: "none", borderRadius: 8, padding: "0 12px", cursor: "pointer" }}>
                            <Plus size={16} />
                          </button>
                        </div>
                      </form>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", color: "var(--alvo-ink)", fontSize: "0.85rem" }}>
                      <thead>
                        <tr style={{ borderBottom: "1px solid var(--alvo-border)", textAlign: "left" }}>
                      ...
                      <tbody>
                        {contributions.map((item) => (
                          <tr key={item.id} style={{ borderBottom: "1px solid var(--alvo-border)" }}>
                            <td style={{ padding: "12px 0" }}>{item.date}</td>
                            <td style={{ padding: "12px 0", fontWeight: 700 }}>{item.category}</td>
                            <td style={{ padding: "12px 0" }}>
                              <span style={{ backgroundColor: "var(--alvo-surface-muted)", border: "1px solid var(--alvo-border)", padding: "2px 8px", borderRadius: 6, fontSize: "0.75rem" }}>{item.method}</span>
                            </td>
                ...
                {activeTab === "academy" && (
                  <div className="animate-entrance">
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                      {[
                        { title: "Curso de Maturidade Cristã V1", pct: 85, color: "var(--getro-primary)", note: "Faltam 2 aulas para a emissão do certificado digital." },
                        { title: "DNA Alvo - Integração e Visão", pct: 100, color: "#10b981", note: "✓ Certificado gerado e anexado ao Getro Pass." },
                      ].map(c => (
                ...
                {activeTab === "serving" && (
                  <div className="animate-entrance">
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
                      ...
                      <div style={{ ...S.card, backgroundColor: "var(--getro-primary-softer)", borderLeft: "4px solid var(--getro-primary)" }}>
                        <span style={{ fontSize: "0.75rem", color: "var(--getro-primary)", textTransform: "uppercase", fontWeight: 800 }}>Próxima Escala</span>
            ...
            <aside style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
              ...
              <div style={S.cardLg}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1rem", fontSize: "0.85rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--alvo-ink-soft)" }}>
                    <Smartphone size={16} style={{ color: "var(--getro-primary)" }} />
                    <span>{person.mobilePhone || "(11) 98765-4321"}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--alvo-ink-soft)" }}>
                    <Mail size={16} style={{ color: "var(--getro-primary)" }} />
                    <span style={{ wordBreak: "break-all" }}>{person.email || "contato@alvo.church"}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--alvo-ink-soft)" }}>
                    <MapPin size={16} style={{ color: "var(--getro-primary)" }} />
              ...
              <div style={S.cardLg}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  {profileHealth.map((item) => (
                    <div key={item.title} style={{ borderBottom: "1px solid var(--alvo-border)", paddingBottom: "0.75rem" }}>
                      <strong style={{ display: "block", fontSize: "0.85rem", color: "var(--getro-primary)" }}>{item.title}</strong>
              ...
              <div style={S.cardLg}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
                  <h3 style={{ fontSize: "1.1rem", fontWeight: 800, color: "var(--alvo-ink)" }}>Getro Pass</h3>
                  <QrCode size={20} style={{ color: "var(--getro-primary)" }} />
                </div>
                <p style={{ ...S.hint, lineHeight: "1.4", marginBottom: "1.5rem", fontSize: "0.8rem" }}>
                  Carteira digital com QR Code criptografado para validação em parceiros locais da comunidade Alvo.
                </p>

                <div style={{ display: "flex", gap: "1rem", alignItems: "center", backgroundColor: "var(--alvo-surface-muted)", padding: "1rem", borderRadius: 16, border: "1px solid var(--alvo-border)" }}>
