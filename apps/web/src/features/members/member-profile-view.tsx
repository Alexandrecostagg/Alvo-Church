...
import { getTribeDisplayLabel, getTribeMinistrySummary } from "@alvo/domain";
import type {
  ContributionType,
  Family,
  FamilyMember,
...
const JOURNEY_STAGES = [
  ...
  {
    code: "cell",
    label: "Frequente em Célula",
    desc: "Participando de pequenos grupos",
  },
  {
    code: "class",
    label: "Classe de Maturidade",
    desc: "Formação de discipulado",
  },
  {
    code: "baptism",
    label: "Batismo / Aliança",
    desc: "Membro formal da igreja",
  },
...
export function MemberProfileView() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const mockPerson =
        (recentPeople as readonly any[]).find((p) => p.id === personId) ||
        (recentPeople as readonly any[])[0];
      if (mockPerson) {
        setPerson(mockPerson);
        if (mockPerson.memberStatus === "visitor") {
          setActiveStage("reception");
        } else if (
          mockPerson.memberStatus === "congregant" ||
          mockPerson.memberStatus === "new_believer"
        ) {
          setActiveStage("cell");
        } else if (mockPerson.memberStatus === "member") {
          setActiveStage("baptism");
        } else if (
          mockPerson.memberStatus === "leader" ||
          mockPerson.memberStatus === "volunteer"
        ) {
    ...
    async function loadProfile() {
      ...
      try {
        const nextPerson = await fetchPersonById(
          firebaseConfig,
          { organizationId },
          personId,
        );
        ...
        if (nextPerson.memberStatus === "visitor") {
          ...
          nextPerson.memberStatus === "congregant" ||
          nextPerson.memberStatus === "new_believer"
        ) {
          setActiveStage("cell");
        } else if (nextPerson.memberStatus === "member") {
          setActiveStage("baptism");
        } else if (
          nextPerson.memberStatus === "leader" ||
          nextPerson.memberStatus === "volunteer"
        ) {
          setActiveStage("leader");
        }
        ...
        const groupTask = (async () => {
          try {
            ...
            const memberships = await fetchGroupMembers(
              firebaseConfig,
              { organizationId },
              allGroups,
              50,
            ...
            if (!cancelled) {
              ...
              setPersonGroup(
                membership
                  ? (allGroups.find((g) => g.id === membership.groupId) ?? null)
                  : null,
              );
            }
          } catch {
        ...
        const servingTask = (async () => {
          const [allAssignments, nextContributions] = await Promise.all([
            fetchServiceAssignments(
              ...
              { organizationId },
              300,
            ).catch(() => [] as ServiceAssignment[]),
            fetchMemberContributionsByPersonId(
              firebaseConfig,
              { organizationId },
              personId,
            ).catch(() => [] as MemberContribution[]),
          ]);
          if (cancelled) return;
          setAssignments(
            allAssignments.filter((item) => item.personId === personId),
          );
        ...
        const familyTask = (async (): Promise<"with-family" | "no-family"> => {
          ...
          if (!cancelled) {
            const familyPersonIds = new Set(
              nextFamilyMembers.map((member) => member.personId),
            );
            setFamily(nextFamily);
            setFamilyMembers(nextFamilyMembers);
            setFamilyPeople(
              nextPeople.filter((item) => familyPersonIds.has(item.id)),
            );
          }
          return "with-family";
        })();
        ...
        const [, , familyOutcome] = await Promise.all([
          groupTask,
          servingTask,
          familyTask,
        ]);
  ...
  const servingSummary = useMemo(() => {
    ...
    const ministryCodes = Array.from(
      new Set(sorted.map((item) => item.ministryCode)),
    );
    const now = new Date();
    const upcoming =
      assignments
        .filter(
          (item) =>
            !["declined", "absent"].includes(item.status) &&
            new Date(item.serviceDate) >= now,
        )
  ...
  const handleAddContribution = async (e: React.FormEvent) => {
    ...
    if (
      ...
      try {
        ...
        const savedId = await addMemberContribution(
          firebaseConfig,
          { organizationId },
          payload,
        );
        ...
        setStatus(
          "Não foi possível salvar a contribuição. Verifique sua conexão e tente novamente.",
        );
        return;
      }
    }
    ...
    setNewAmount("");
    setStatus(
      `Contribuição de R$ ${amountVal.toFixed(2)} cadastrada com sucesso!`,
    );
  };

  return (
    <main
      className="form-page profile-page animate-entrance"
      style={{ maxWidth: 1400, padding: "2rem" }}
    >
      {/* Cabeçalho do Perfil */}
      <section
      ...
      >
        <div>
          ...
          <p className="eyebrow" style={{ color: "var(--alvo-accent)" }}>
            Ficha Inteligente
          </p>
          <h1>{fullName}</h1>
          <p
          ...
          >
            Visão pastoral integrada: acompanhe o progresso da jornada da alma,
            a participação nas células, doações fiéis, voluntariado e trilha
            acadêmica de forma unificada.
          </p>
        </div>
        <aside
          style={{
      ...
      {person ? (
          ...
          >
            ...
            <article style={S.card}>
              ...
              <strong
                style={{
                  ...S.value,
                  color: person.partnerBenefitsEnabled
                    ? "#10b981"
                    : "var(--alvo-ink-soft)",
                }}
              >
            ...
            <article style={S.card}>
              ...
              <strong
                style={{
                  ...S.value,
                  color: person.consentLgpdAt ? "#10b981" : "#d97706",
                }}
              >
                {person.consentLgpdAt ? "Confirmado" : "Pendente"}
              </strong>
              <p style={S.hint}>
                {person.consentLgpdAt
                  ? "Termo de consentimento assinado"
                  : "Termo de consentimento ainda não assinado"}
              </p>
            </article>
            ...
            <Link href="/tribes" style={{ textDecoration: "none" }}>
              <article
                style={{
                  ...S.card,
                  backgroundColor: "var(--alvo-accent-soft)",
                  borderLeft: "4px solid var(--alvo-accent)",
                  height: "100%",
                }}
              >
                <span
                  style={{ fontSize: "0.75rem", color: "var(--alvo-accent)" }}
                >
                  Tribo (vocação ministerial)
                </span>
                <strong style={S.value}>
                  {person.tribePrimaryCode
                    ? getTribeDisplayLabel(person.tribePrimaryCode)
                    : "Não classificada"}
                </strong>
            ...
            <Link href="/groups" style={{ textDecoration: "none" }}>
              ...
              >
                ...
                <p style={{ ...S.hint, color: "#059669" }}>
                  {personGroup
                    ? [personGroup.meetingTime, personGroup.city]
                        .filter(Boolean)
                        .join(" · ") || "Detalhes na página de Células"
          ...
          <section style={{ marginTop: "2.5rem" }}>
            <div style={S.cardLg}>
              ...
              >
                ...
                >
                  Estágio atual:{" "}
                  {JOURNEY_STAGES.find((s) => s.code === activeStage)?.label ||
                    activeStage}
                </span>
              </div>
              ...
              >
                ...
                {JOURNEY_STAGES.map((stage, idx) => {
                  const isCompleted =
                    JOURNEY_STAGES.findIndex((s) => s.code === activeStage) >=
                    idx;
                    ...
                    >
                      <div
                        style={{
                          ...
                          border: isActive
                            ? "4px solid var(--alvo-accent-soft)"
                            : "2px solid var(--alvo-line)",
                          display: "flex",
                          alignItems: "center",
          ...
          >
                  ...
                  <div className="animate-entrance">
                    ...
                    >
                      <div>
                        ...
                        >
                          Líder Responsável:{" "}
                          <strong>Patrícia Albuquerque</strong>
                        </p>
                      </div>
                      <span
                        style={{
                    ...
                    >
                      Esta pessoa faz parte da Célula Betel, que se reúne às{" "}
                      <strong style={S.ink}>quartas-feiras às 20:00</strong> no
                      bairro Campestre.
                    </p>
                    ...
                    <div style={{ marginTop: "2rem" }}>
                      ...
                      >
                        Rastreador de Presenças (Últimas 4 reuniões):
                      </h4>
                      ...
                      >
                        ...
                        <div style={{ display: "flex", gap: "0.5rem" }}>
                          <select
                            value={newCategory}
                            onChange={(e) =>
                              setNewCategory(e.target.value as ContributionType)
                            }
                            style={{
                              flex: 1,
                              padding: "8px",
                        ...
                        {contributions.length === 0 ? (
                          ...
                          contributions.map((item) => (
                            ...
                            >
                              ...
                              <td
                                style={{ padding: "12px 0", fontWeight: 700 }}
                              >
                ...
                {activeTab === "academy" && (
                  <div className="animate-entrance">
                    ...
                    >
                      {[
                        {
                          title: "Curso de Maturidade Cristã V1",
                          pct: 85,
                          color: "var(--alvo-accent)",
                          note: "Faltam 2 aulas para a emissão do certificado digital.",
                        },
                        {
                          title: "DNA Esdras - Integração e Visão",
                          pct: 100,
                          color: "#10b981",
                          note: "✓ Certificado gerado e anexado ao Esdras Passe.",
                        },
                      ].map((c) => (
                  ...
                  <div className="animate-entrance">
                    ...
                    {servingSummary ? (
                      ...
                      >
                        ...
                        >
                          ...
                          <strong style={{ ...S.value, fontSize: "1.25rem" }}>
                            {MINISTRY_NAMES[
                              servingSummary.latest.ministryCode
                            ] ?? servingSummary.latest.ministryCode}
                          </strong>
                          <p style={{ ...S.hint, fontSize: "0.8rem" }}>
                            Função:{" "}
                            <strong style={S.ink}>
                              {servingSummary.latest.role}
                            </strong>
                          </p>
                          {servingSummary.ministryCodes.length > 1 && (
                            <p style={S.hint}>
                              Também serve em:{" "}
                              {servingSummary.ministryCodes
                                .slice(1)
                        ...
                        >
                          ...
                          {servingSummary.upcoming ? (
                            <>
                              <strong
                                style={{ ...S.value, fontSize: "1.25rem" }}
                              >
                                {MINISTRY_NAMES[
                                  servingSummary.upcoming.ministryCode
                                ] ?? servingSummary.upcoming.ministryCode}
                              </strong>
                              <p style={{ ...S.hint, fontSize: "0.8rem" }}>
                                Data:{" "}
                                <strong style={S.ink}>
                                  {formatDateTime(
                                    servingSummary.upcoming.serviceDate,
                                  )}
                                </strong>
                              </p>
                              {["confirmed", "present"].includes(
                                servingSummary.upcoming.status,
                              ) ? (
                                <p
                                  style={{
                                    ...S.hint,
                      ...
                      <div style={S.cardMuted}>
                        <strong style={{ ...S.value, fontSize: "1.1rem" }}>
                        ...
                        >
                          Esta pessoa ainda não foi escalada em nenhum
                          ministério. Use a área de Escalas para incluí-la em
                          uma equipe de voluntariado.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            >
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ wordBreak: "break-all" }}>
                      {person.email || "contato@plataformaesdras.com.br"}
                    </span>
                  </div>
                  <div
                    style={{
              ...
              <div style={S.cardLg}>
                ...
                >
                  Carteira digital com QR Code criptografado para validação em
                  parceiros locais da comunidade Esdras.
                </p>
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {person.memberCardCode || "ALVO-002-391"}
                    </strong>
                  </div>
                </div>
              </div>
        ...
        <section
          ...
          style={{ marginTop: "2rem", ...S.cardLg }}
        >
          <h2 style={S.ink}>{status}</h2>
          <p style={{ ...S.soft, marginTop: "1rem" }}>
            Se você acabou de cadastrar o membro, retorne à lista de busca e
            verifique a ficha novamente.
          </p>
          <Link
...
function getProfileHealth(person: Person) {
  ...
  if (!person.primaryFamilyId) {
    items.push({
      ...
      detail:
        "Sem grupo familiar vinculado. Ligue esta pessoa a uma casa para rastreamento de núcleo domiciliar.",
    });
  }
  ...
  if (!person.whatsappPhone && !person.mobilePhone) {
    items.push({
      ...
      detail:
        "Nenhum celular ou WhatsApp registrado na base de dados sensível.",
    });
  }
  ...
  if (!person.consentLgpdAt) {
    items.push({
      ...
      detail:
        "Membro ainda não tem assinatura de conformidade com dados protegidos.",
    });
  }
  ...
  if (!items.length) {
    items.push({
      ...
      detail:
        "Todas as tarefas cadastrais e de privacidade estão resolvidas para esta pessoa.",
    });
  }
