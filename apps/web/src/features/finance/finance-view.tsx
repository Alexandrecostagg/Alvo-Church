...
const initialGivingGoals: GivingGoal[] = [
  {
    ...
    description:
      "Meta para ampliação das galerias laterais do auditório e modernização do sistema de som.",
    targetAmount: 100000,
    raisedAmount: 64500,
    category: "Obras",
  },
  {
    ...
    description:
      "Arrecadação mensal de mantimentos e kits de higiene para famílias vulneráveis.",
    targetAmount: 12000,
    raisedAmount: 9800,
    category: "Social",
  },
...
export function FinanceView() {
  const { configured, user, organizationId, firebaseConfig, tenantRuntime } =
    useAppAuth();
  ...
  async function openReceipt(receiptId: string) {
    ...
    try {
      const r = await fetchContributionReceipt(
        firebaseConfig,
        { organizationId },
        receiptId,
      );
  ...
  async function handleConfirmContribution(id: string) {
    ...
    setConfirmingId(id);
    try {
      await confirmMemberContribution(
        firebaseConfig,
        { organizationId },
        id,
        user.uid,
  ...
  const reloadTransactions = useCallback(async () => {
    ...
    try {
      const list = await fetchFinancialTransactions(
        firebaseConfig,
        { organizationId },
        500,
      );
  ...
  const currentReport = useMemo(() => {
    ...
    const kindToCategory: Record<
      FinancialTransaction["kind"],
      "Entrada" | "Saída" | "Missões"
    > = {
      income: "Entrada",
      expense: "Saída",
      missions: "Missões",
    ...
    const incomeMonth = sumBy(
      (e) => e.category !== "Saída" && isMonth(e.createdAt),
    );
    const expensesMonth = sumBy(
      (e) => e.category === "Saída" && isMonth(e.createdAt),
    );
    const missionsMonth = sumBy(
      (e) => e.category === "Missões" && isMonth(e.createdAt),
    );
  ...
  const filteredEntries = useMemo(() => {
    return currentReport.entries.filter(
      (entry) =>
  ...
  const monthlySeries = useMemo(() => {
    ...
    for (const e of currentReport.entries) {
      ...
      if (e.category === "Saída") cur.expense += e.amount;
  ...
  const handleCreateEntry = async (e: React.FormEvent) => {
    ...
    const kind: FinancialTransaction["kind"] =
      newEntry.category === "Entrada"
        ? "income"
        : newEntry.category === "Saída"
          ? "expense"
          : "missions";
    try {
    ...
    <main className="finance-workbench animate-entrance">
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    ESDRAS CONTABILIDADE
                  </strong>
                  <span style={{ fontSize: "0.8rem", color: "#64748b" }}>
                    Relatório Geral de Transparência Financeira
                  </span>
                </div>
                <div style={{ textAlign: "right" }}>
                  <strong style={{ fontSize: "0.95rem", color: "#0f172a" }}>
                    Competência: {currentReport.month}
                  </strong>
                  <span
                    style={{
                      fontSize: "0.75rem",
              ...
              >
                <div>
                  ...
                  >
                    {formatCurrency(currentReport.income)}
                  </strong>
                </div>
                <div>
                  <span
                  ...
                  >
                    {formatCurrency(currentReport.expenses)}
                  </strong>
                </div>
                <div>
                  <span
                  ...
                  >
                    {formatCurrency(currentReport.missions)}
                  </strong>
                </div>
              </div>
              ...
              >
                <strong style={{ fontSize: "1.1rem", color: "#0f172a" }}>
                  Saldo Líquido Operacional em Caixa
                </strong>
                <strong style={{ fontSize: "1.25rem", color: "#f97316" }}>
              ...
              >
                Detalhamento das Operações Recentes:
              </strong>
              <div
                style={{
                  display: "flex",
                  ...
                  >
                    <span>
                      ...
                      <small style={{ color: "#64748b", display: "block" }}>
                        Categoria: {ent.category}
                      </small>
                    </span>
                    <strong
                      style={{
                        color: ent.category === "Saída" ? "#dc2626" : "#16a34a",
                      }}
                    >
                      {ent.category === "Saída" ? "-" : "+"}{" "}
              ...
              >
                <div style={{ textAlign: "center", width: "240px" }}>
                  <div
                    style={{ borderBottom: "1px solid #94a3b8", height: 20 }}
                  />
                  <span
                    style={{
                      fontSize: "0.7rem",
      ...
      <header className="finance-hero topbar">
        <div className="topbar-content">
          ...
          <p>
            Controle entradas, saídas, missões, metas e contribuições por PIX em
            uma visão clara para liderança e transparência.
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            {pendingContributions.length > 0 && (
              ...
              >
                {pendingContributions.length} pendente
                {pendingContributions.length > 1 ? "s" : ""} de confirmação
              </span>
            )}
          </div>
          ...
          {pendingContributions.length === 0 ? (
            ...
            >
              Nenhuma contribuição pendente — {contributions.length} registro
              {contributions.length > 1 ? "s" : ""} no total, todos conferidos.
            </p>
          ) : (
      ...
      >
        ...
        <div
          className="finance-left-column"
          style={{ display: "flex", flexDirection: "column", gap: "2rem" }}
        >
          {/* Nubank Card de Caixa e Indicadores */}
          <div
            className="finance-main-card antigravity-float"
            style={{ width: "100%" }}
          >
            ...
            <div className="main-metrics">
              ...
              <div className="metric-primary">
                ...
                <p style={{ opacity: 0.6, fontSize: "0.85rem", marginTop: 4 }}>
                  Saldo operacional líquido disponível
                </p>
              </div>
            </div>
            ...
            >
              <button
                ...
                style={{
                  ...
                  border: "none",
                  color:
                    activeTab === "chart" ? "#f97316" : "rgba(255,255,255,0.6)",
                  fontWeight: 700,
                  cursor: "pointer",
                  fontSize: "0.9rem",
                  display: "flex",
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  color:
                    activeTab === "goals" ? "#f97316" : "rgba(255,255,255,0.6)",
                  fontWeight: 700,
                  cursor: "pointer",
                  fontSize: "0.9rem",
                  display: "flex",
              ...
              {activeTab === "chart" && (
                <div className="animate-entrance">
                  ...
                  >
                    ...
                    >
                      Últimos {monthlySeries.length} meses
                    </span>
                  </div>
                  {monthlySeries.length === 0 ? (
                    ...
                    >
                      Sem lançamentos ainda. Registre entradas e saídas para ver
                      a evolução.
                    </p>
                  ) : (
                    <div
                      ...
                      {(() => {
                        ...
                        return monthlySeries.map((m, i) => (
                          ...
                          >
                            ...
                            >
                              <div
                                title={"Entradas " + formatCurrency(m.income)}
                                style={{
                                  width: 14,
                              ...
                              />
                              <div
                                title={"Saídas " + formatCurrency(m.expense)}
                                style={{
                                  width: 14,
                      ...
                      {expenseBreakdown.map((it, i) => (
                        <div key={i}>
                          ...
                          >
                            ...
                            >
                              {formatCurrency(it.amount)} · {it.pct}%
                            </strong>
                          </div>
                          <div
                            style={{
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span>
                            Arrecadado:{" "}
                            <strong>{formatCurrency(goal.raisedAmount)}</strong>
                          </span>
                          <span>
                            Meta Geral:{" "}
          ...
          <section
            className="finance-entries-panel"
            style={{
              margin: 0,
              width: "100%",
              background: "rgba(30, 41, 59, 0.4)",
          ...
          >
            ...
            <div
              className="entries-table-wrapper"
                ...
                <tbody>
                  {filteredEntries.map((entry) => (
                    ...
                    >
                      ...
                      >
                        <span
                          className={`entry-badge ${entry.category === "Entrada" ? "income" : entry.category === "Missões" ? "missions" : "expense"}`}
                          style={{
                            backgroundColor:
                              entry.category === "Entrada"
                                ? "rgba(16,185,129,0.15)"
                                : entry.category === "Missões"
                        ...
                        >
                          ...
                          {entry.source === "tx" && (
                            <button
                              type="button"
                              onClick={() =>
                                handleDeleteEntry(entry.id, entry.source)
                              }
        ...
        <aside
          className="finance-gateway-panel"
        ...
        >
          ...
          >
            ...
            >
              Qualquer pessoa (mesmo não-membro) contribui via PIX em segundos,
              sem baixar o app nem se cadastrar. O dinheiro cai direto na conta
              da igreja.
            </p>
            {orgSlug ? (
              <Link
              ...
              >
                Configure o slug público da igreja em Configurações.
              </span>
            )}
            <small
              style={{ color: "rgba(255,255,255,0.45)", fontSize: "0.78rem" }}
            >
              Compartilhe o link/QR. As doações captadas aparecem em{" "}
