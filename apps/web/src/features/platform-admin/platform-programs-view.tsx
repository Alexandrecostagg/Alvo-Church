    ...
    return (
      <div style={{ display: "grid", gap: 10 }}>
        <p style={{ fontSize: 14, fontWeight: 700, margin: "0 0 4px" }}>
          {form.id ? "Editar trilha" : "Nova trilha"}
        </p>
        <Field label="Título">
          <input
            value={form.title}
            ...
            style={inputStyle}
            placeholder="Ex.: Formação de Líderes de Célula"
          />
        </Field>
        <Field label="Descrição">
          <textarea
            ...
            placeholder="O que a igreja recebe nesta trilha"
          />
        </Field>
        <div
        ...
        >
          ...
          <Field label="Cargo (opcional)">
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, instructorTitle: e.target.value })
              }
              style={inputStyle}
        ...
        >
          ...
          <Field label="Preço (R$)">
            <input
              ...
              value={form.priceBRL}
              onChange={(e) =>
                setForm({ ...form, priceBRL: Number(e.target.value) })
              }
              style={inputStyle}
            />
          </Field>
  ...
  return (
    <div>
      ...
      >
        ...
        >
          Trilhas vendidas às igrejas na Loja de Capacitação. {programs.length}{" "}
          cadastrada{programs.length === 1 ? "" : "s"}.
        </p>
        {!form && (
                ...
                <>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            padding: "2px 8px",
                            borderRadius: 6,
                            background: p.isPublished ? "#E1F5EE" : "#F1EFE8",
                            color: p.isPublished ? "#085041" : "#7a756a",
                          }}
                        >
                      ...
                      >
                        R${" "}
                        {p.priceBRL.toLocaleString("pt-BR", {
...
function LessonsEditor({ programId }: { programId: string }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetchTrainingLessons(firebaseConfig, programId)
      .then(setLessons)
  ...
  async function saveLessonPatch(
    lesson: TrainingLesson,
    patch: Partial<Pick<TrainingLesson, "materialUrl" | "content">>,
  ) {
    const nextMaterial =
      "materialUrl" in patch
        ? patch.materialUrl?.trim() || undefined
        : lesson.materialUrl;
    const nextContent =
      "content" in patch ? patch.content?.trim() || undefined : lesson.content;
    const unchanged =
    ...
    setBusy(true);
    try {
      await saveTrainingLesson(firebaseConfig, {
        ...lesson,
        materialUrl: nextMaterial,
        content: nextContent,
      });
      load();
    } finally {
  ...
  async function removeLesson(id: string) {
    setBusy(true);
    try {
      await deleteTrainingLesson(firebaseConfig, programId, id);
      load();
    } finally {
      setBusy(false);
    }
        ...
        <>
          ...
          <div style={{ display: "grid", gap: 6 }}>
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "2fr 2fr 90px 1fr auto",
                gap: 6,
                alignItems: "center",
              }}
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setDraft({ ...draft, moduleId: e.target.value })
                }
                placeholder="Módulo (opc.)"
                style={{ ...inputStyle, fontSize: 12 }}
              />
              <button onClick={addLesson} disabled={busy} style={primaryBtn}>
                <Plus size={14} />
              </button>
            </div>
            <input
              ...
              onChange={(e) =>
                setDraft({ ...draft, materialUrl: e.target.value })
              }
