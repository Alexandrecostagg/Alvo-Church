  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 4, padding: "16px 20px 0" }}>
        <TabButton active={tab === "serving"} onClick={() => setTab("serving")} icon={Handshake} label="Escalas" />
        <TabButton active={tab === "worship"} onClick={() => setTab("worship")} icon={Music} label="Louvor & Cifras" />
      </div>
