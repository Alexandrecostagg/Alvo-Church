...
const DEFAULT_TEAMS: MinistryTeam[] = ministryTeams.map((t) => ({
  code: t.code,
  name: t.name,
  summary: t.summary,
  target: t.target,
...
export function ServingView() {
  ...
  const [auditLogs, setAuditLogs] = useState<
    Array<{ time: string; text: string; type: "success" | "info" | "warning" }>
  >([]);

  const [assignMode, setAssignMode] = useState<"members" | "new">("members");
  const [selectedMinistryCode, setSelectedMinistryCode] =
    useState<string>("reception");
  const [status, setStatus] = useState("Carregando escalas...");

  // Interactive Reminders
  const [selectedAssignmentForReminder, setSelectedAssignmentForReminder] =
    useState<ServiceAssignment | null>(null);
  const [whatsappTemplate, setWhatsappTemplate] = useState(
    "Olá [Nome]! Confirmando a sua escala no ministério de [Ministerio] para o dia [Data] no papel de [Role]. Confirma sua presença?",
  );
  ...
  // Swap Form States
  const [requestingSwapForAssignmentId, setRequestingSwapForAssignmentId] =
    useState<string | null>(null);
  const [swapReplacementPersonId, setSwapReplacementPersonId] =
    useState<string>("");
  ...
  useEffect(() => {
    ...
    async function loadServingData() {
      ...
      try {
        const [nextPeople, nextAssignments, nextSwaps, nextTeams] =
          await Promise.all([
            cachedFetchPeople(firebaseConfig, { organizationId }, 160),
            fetchServiceAssignments(firebaseConfig, { organizationId }, 160),
            fetchScheduleSwapRequests(firebaseConfig, { organizationId }).catch(
              () => [] as ScheduleSwapRequest[],
            ),
            fetchServiceTeams(firebaseConfig, { organizationId }).catch(
              () => [] as ServiceTeam[],
            ),
          ]);
        ...
        if (nextTeams.length > 0) {
          finalTeams = nextTeams.map((t) => ({
            code: t.code,
            name: t.name,
            summary: t.summary ?? "",
            target: t.targetVolunteers ?? 0,
          }));
        } else {
        ...
        if (!cancelled) {
          setStatus(
            "Não foi possível carregar pessoas. Verifique sua conexão e tente novamente.",
          );
        }
      }
    }
  ...
  const selectedMinistry =
    teams.find((team) => team.code === selectedMinistryCode) ?? teams[0];
  ...
  const coverage = Math.round(
    (confirmedCount / Math.max(assignments.length, 1)) * 100,
  );
  ...
  async function handleAssignmentStatus(
    ...
    const updatedAssignment = applyAssignmentStatus(
      currentAssignment,
      nextStatus,
      responseNote,
    );
    ...
    setAuditLogs((prev) => [
      {
        time: timeString,
        text: `${personName} foi marcado como ${statusLabel}.`,
  ...
  async function handleQuickAssign(person: Person, role = "Apoio") {
    ...
    if (conflict) {
      const conflictingTeam =
        teams.find((t) => t.code === conflict.ministryCode)?.name ||
        conflict.ministryCode;
    ...
    setAuditLogs((prev) => [
      {
        time: timeString,
        text: `${getFullName(person)} foi escalado(a) como ${role} em ${selectedMinistry.name}.`,
        type: "info",
      },
      ...prev,
    ]);
    ...
    if (
      ...
      try {
        ...
        setStatus(
          `${getFullName(person)} escalado em ${selectedMinistry.name}.`,
        );
      } catch (error) {
        setStatus(
          error instanceof Error
  ...
  async function handleServantRegistration(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      try {
        ...
        setStatus(
          `${getFullName(servant)} cadastrado como voluntário e escalado em ${selectedMinistry.name}.`,
        );
      } catch (error) {
        setStatus(
          error instanceof Error
      ...
      setStatus(
        `${getFullName(servant)} cadastrado localmente e escalado em ${selectedMinistry.name}.`,
      );
    }
  ...
  const handleSendReminder = (assignment: ServiceAssignment) => {
    ...
    const formattedDate = new Date(assignment.serviceDate).toLocaleDateString(
      "pt-BR",
    ...
    setAuditLogs((prev) => [
      {
        time: timeString,
        text: `Lembrete via WhatsApp disparado para ${getFullName(person)}.`,
        type: "success",
      },
      ...prev,
    ]);
  ...
  async function handleRequestSwap(
    ...
    setAuditLogs((prev) => [
      {
        time: timeString,
        text: `Solicitada troca: ${requestorName} por ${replacementName}.`,
        type: "info",
      },
      ...prev,
    ]);
  ...
  async function handleAcceptSwap(swapId: string) {
    ...
    try {
      ...
      setAssignments((current) =>
        current.map((a) =>
          a.id === requestorAssignment.id ? requestorAssignment : a,
        ),
      );
      ...
      setSwapRequests((current) =>
        current.map((s) => (s.id === swapId ? updatedSwap : s)),
      );

      const requestor = people.find((p) => p.id === swap.requestorPersonId);
      const replacement = people.find(
        (p) =>
          p.id === (swap.proposedReplacementPersonId || swap.targetPersonId),
      );
      const requestorName = requestor ? getFullName(requestor) : "Voluntário";
      const replacementName = replacement
        ? getFullName(replacement)
      ...
      setAuditLogs((prev) => [
        {
          time: timeString,
          text: `Troca aprovada: ${replacementName} assume o lugar de ${requestorName}.`,
          type: "success",
        },
        ...prev,
      ]);
  ...
  async function handleDeclineSwap(swapId: string) {
    ...
    setAuditLogs((prev) => [
      {
        time: timeString,
        text: `Troca recusada para a escala de ${requestorName}.`,
        type: "warning",
      },
      ...prev,
    ]);
  ...
  async function handleRemoveAssignment(assignmentId: string) {
    const confirmRemove = window.confirm(
      "Deseja realmente remover este voluntário desta escala?",
    );
    ...
    setAuditLogs((prev) => [
      {
        time: timeString,
        text: `${personName} foi removido(a) da escala.`,
        type: "warning",
      },
      ...prev,
    ]);
    ...
    <div className="page-root serving-page">
      <header className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">
            Gerencie equipes, confirme presenças e acompanhe cobertura em tempo
            real
          </p>
        </div>
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          ...
          <div className="stat-body">
            <span className="stat-label">Banco de voluntários</span>
            <span className="stat-value">
              {availablePeople.length || people.length}
            </span>
          </div>
        </div>
        <div className="stat-card">
        ...
        >
          ...
          >
            {swapRequests
              ...
              .map((swap) => {
                const requestor = people.find(
                  (p) => p.id === swap.requestorPersonId,
                );
                const replacement = people.find(
                  (p) =>
                    p.id ===
                    (swap.proposedReplacementPersonId || swap.targetPersonId),
                );
                const assignment = assignments.find(
                  (a) => a.id === swap.assignmentId,
                );

                const requestorName = requestor
                  ? getFullName(requestor)
                  : swap.requestorPersonId;
                const replacementName = replacement
                  ? getFullName(replacement)
                  : "Substituto";
                const teamName = assignment
                  ? teams.find((t) => t.code === assignment.ministryCode)
                      ?.name || assignment.ministryCode
                  : "Ministério";
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      >
                        {requestorName} solicita substituição por{" "}
                        {replacementName}
                      </p>
                      <p
                        style={{
                      ...
                      >
                        Ministério: {teamName}{" "}
                        {assignment ? `| Função: ${assignment.role}` : ""}
                      </p>
                      {swap.note && (
                        <p
                          style={{
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            >
              Matriz de Escalas por Culto
            </h3>
          </div>
          <span style={{ fontSize: "0.8rem", color: "var(--alvo-ink-soft)" }}>
            Clique em um domingo para ver e gerenciar a escala respectiva
          </span>
        </div>
        <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
          ...
          <div
            style={{
              ...
              padding: "1rem",
              borderRadius: "16px",
              border: !nextSundays.some(
                (s) => s.dateString === selectedDateFilter,
              )
                ? "2.5px solid var(--alvo-accent)"
                : "1px solid var(--alvo-line)",
              background: !nextSundays.some(
                (s) => s.dateString === selectedDateFilter,
              )
                ? "var(--alvo-accent-soft)"
                : "rgba(255, 255, 255, 0.35)",
      ...
      >
          ...
          >
            {teams.map((team) => {
              const teamAssignments = assignments.filter(
                (assignment) =>
                  assignment.ministryCode === team.code &&
                  assignment.serviceDate.startsWith(selectedDateFilter),
              );
        ...
        <article className="panel" style={{ padding: "2rem" }}>
          ...
          >
            ...
            <p style={{ color: "var(--alvo-ink-soft)", margin: "4px 0 0" }}>
              {nextActionLabel}
            </p>
          </div>
            ...
            {selectedAssignments.length ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: "0.5rem" }}>
                        ...
                        <button
                          onClick={() =>
                            setRequestingSwapForAssignmentId(assignment.id)
                          }
                          style={{
                    ...
                    {requestingSwapForAssignmentId === assignment.id && (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            Selecione o Voluntário de Substituição:
                          </label>
                          <select
                            value={swapReplacementPersonId}
                            onChange={(e) =>
                              setSwapReplacementPersonId(e.target.value)
                            }
                            style={{
                              padding: "8px 12px",
                              borderRadius: "10px",
              ...
              <div
                style={{
                  ...
                  padding: "3rem",
                  background: "rgba(255, 255, 255, 0.35)",
                  border: "2px dashed var(--alvo-line)",
                  borderRadius: "20px",
                }}
              >
                <Handshake
                ...
                />
                <strong style={{ display: "block", color: "var(--alvo-ink)" }}>
                  Nenhum escalado para este dia
                </strong>
                <p
                  style={{
                    color: "var(--alvo-ink-soft)",
                ...
                >
                  Selecione sugestões por Tribo de Dons abaixo para escalar
                  voluntários.
                </p>
              </div>
            )}
          ...
          {suggestedPeople.length > 0 && (
            ...
            >
              ...
              >
                ...
                >
                  Sugestões Inteligentes por Tribos
                </h4>
              </div>
              <div
                style={{
            ...
            >
              {filteredCandidatePeople.slice(0, 15).map((p) => (
                <button
                  key={p.id}
                  onClick={() =>
                    handleQuickAssign(p, customRoleInput.trim() || undefined)
                  }
                  style={{
                    background: "white",
                    border: "1px solid var(--alvo-line)",
        ...
        >
          ...
          <div className="panel" style={{ padding: "1.5rem" }}>
            ...
            <form
              onSubmit={handleServantRegistration}
              style={{
                display: "flex",
                flexDirection: "column",
                gap: "0.75rem",
          ...
          <div className="panel" style={{ padding: "1.5rem" }}>
            ...
            >
              {auditLogs.map((log, idx) => (
                ...
                >
                  <span
                    style={{ color: "var(--alvo-accent)", fontWeight: 800 }}
                  >
                    {log.time}
                  </span>{" "}
                  -{" "}
                  <span style={{ color: "var(--alvo-ink-soft)" }}>
                    {log.text}
                  </span>
                </div>
              ))}
      ...
      {selectedAssignmentForReminder && (
        ...
        >
          ...
          >
            ...
            >
              Personalize a mensagem abaixo antes de disparar o WhatsApp
              automático para o voluntário pendente.
            </p>
            ...
            <div style={{ display: "flex", gap: "1rem" }}>
              ...
              <button
                onClick={() =>
                  handleSendReminder(selectedAssignmentForReminder)
                }
                style={{
                  flex: 1,
                  background: "var(--alvo-green)",
