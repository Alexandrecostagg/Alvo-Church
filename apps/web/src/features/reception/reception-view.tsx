...
export function ReceptionView() {
  ...
  const [capturedVisitors, setCapturedVisitors] =
    useState<CapturedVisitor[]>(demoVisitors);
  ...
  const [activeTemplateVisitor, setActiveTemplateVisitor] =
    useState<CapturedVisitor | null>(null);
      ...
      try {
        ...
        if (nextIntakes.length > 0) {
          const journeyById = new Map(
            nextJourneys.map((journey) => [journey.id, journey]),
          );
          setCapturedVisitors(
            nextIntakes
              .filter((intake) => intake.status !== "archived")
              .map((intake) =>
                mapIntakeToCapturedVisitor(
                  intake,
                  intake.journeyId
                    ? journeyById.get(intake.journeyId)
                    : undefined,
                ),
              ),
          );
  ...
  const handleKioskSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const source =
      (formData.get("kiosk_source") as string) || "Passando na rua";
  ...
  const handleSendWhatsAppMessage = async () => {
    ...
    window.open(
      `https://web.whatsapp.com/send?phone=55${phoneClean}&text=${textEncoded}`,
      "_blank",
    );

    // Marca como completado reativamente no painel
    setPreparedCommunicationIds((current) =>
      current.includes(activeTemplateVisitor.id)
        ? current
        : [...current, activeTemplateVisitor.id],
    );
    setCapturedVisitors((current) =>
      current.map((visitor) =>
    ...
    if (firebaseConnected) {
      try {
        await updateVisitorIntakeStatus(
          firebaseConfig,
          { organizationId },
          {
        ...
        setStatus(
          "WhatsApp preparado localmente. Não foi possível atualizar o status na nuvem agora.",
        );
    ...
    setStatus(
      `WhatsApp de boas-vindas preparado para ${activeTemplateVisitor.name}.`,
    );
    setActiveTemplateVisitor(null);
  };
  ...
  const handleMarkGreetingComplete = async (visitorId: string) => {
    ...
    setStatus(
      visitor
        ? `${visitor.name} marcado como saudado no altar.`
        : "Visitante marcado como saudado.",
    );
  };
  ...
  return (
    <main className="form-page reception-page animate-entrance">
        ...
        >
          ...
          {kioskStep === "form" ? (
            ...
            >
              ...
              >
                Ficamos muito contentes com sua presença. Preencha seus dados
                rápidos para podermos te acolher com carinho hoje!
              </p>
              ...
              >
                ...
                >
                  ...
                  >
                    Como você conheceu a Plataforma Esdras?
                  </label>
                  <select
                    name="kiosk_source"
                    style={{
                  ...
                  >
                    <option value="Convite de membro">
                      Fui convidado por um amigo / familiar
                    </option>
                    <option value="Instagram">Instagram / Redes Sociais</option>
                    <option value="Passando na rua">
                      Moro perto / vi o templo
                    </option>
                    <option value="Evento especial">
                      Vim para um evento especial
                    </option>
                  </select>
                </div>
            ...
            >
              ...
              >
                Muito obrigado, <strong>{lastCreated?.name}</strong>! Já
                registramos a sua chegada.
              </p>
              <p
                style={{
                  fontSize: "0.95rem",
              ...
              >
                Procure nossa equipe de acolhimento na saída para retirar um
                presente especial! 🎁
              </p>
              <div
                style={{
        ...
        <div className="pulpit-live-feed animate-entrance">
          ...
          <div className="pulpit-header">
            <div>
              ...
              <p>
                Lista limpa para saudação pública, com observações importantes
                da recepção.
              </p>
            </div>
          ...
          <div className="pulpit-card-grid">
            {celebrationGreetingVisitors.length > 0 ? (
              celebrationGreetingVisitors.map((visitor, idx) => (
                <div key={visitor.id} className="pulpit-card">
                  <div>
                    <span className="pulpit-card-number">#{idx + 1}</span>
                    <h2 className="pulpit-card-title">{visitor.name}</h2>
                    {visitor.note && (
                  ...
                  <div className="pulpit-card-footer">
                    <span className="pulpit-card-source">
                      Origem: <strong>{visitor.source}</strong>
                    </span>
                    <button
              ...
              <div className="pulpit-empty-state">
                ...
                <p>
                  Quando a recepção preparar nomes para saudação, eles
                  aparecerão aqui.
                </p>
              </div>
            )}
          ...
          >
            ...
            >
              <p style={{ fontSize: "0.85rem", color: "var(--alvo-ink-soft)" }}>
                Selecione ou edite a mensagem abaixo para enviar diretamente ao
                celular de <strong>{activeTemplateVisitor.name}</strong> (
                {activeTemplateVisitor.phone}):
              </p>
              ...
              <div style={{ display: "flex", gap: "1rem", marginTop: "1rem" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    gap: 6,
                    border: "none",
                    opacity: activeTemplateVisitor.phone ? 1 : 0.55,
                  }}
                >
                  <Send size={16} />
                  {activeTemplateVisitor.phone
        ...
        <div className="reception-header-main-row">
          <div className="reception-header-left">
            ...
            <p className="desc-premium">
              Uma tela rápida para tablet, celular ou notebook na entrada:
              captura o visitante, inicia a jornada pastoral e aciona follow-ups
              e altar.
            </p>
          </div>
      ...
      <section className="reception-kpi-grid">
        <div className="reception-kpi-card">
          ...
          <small className="reception-kpi-desc text-green">
            {cloudVisitorCount
              ? `${cloudVisitorCount} sincronizados na nuvem`
              : `${localVisitorCount} no painel local`}
          </small>
        </div>
        <div className="reception-kpi-card">
        ...
        <div className="reception-kpi-card">
          ...
          <small className="reception-kpi-desc text-green">
            {firebaseConnected
              ? "Sincronizadas no Firebase"
              : "Aguardando conexão Firebase"}
          </small>
        </div>
      </section>
      ...
      <section className="reception-command-grid">
        ...
        <form
          className="visitor-form reception-capture-card"
          onSubmit={handleVisitorCaptureSubmit}
        >
          ...
          <p className="form-status" style={{ color: "var(--alvo-blue)" }}>
            ✨ {status}
          </p>
        </form>
        ...
        <article className="reception-route-card">
          <div className="section-heading">
            <div>
              ...
              <h2 style={{ color: "var(--alvo-ink)" }}>
                Fluxo da Porta ao Cuidado
              </h2>
            </div>
          ...
          {lastCreated ? (
            <div
              className="reception-success-card animate-entrance"
              style={{
            ...
            >
              <div
                ...
                style={{ backgroundColor: "var(--alvo-blue)" }}
              >
              ...
              <div className="success-content">
                ...
                {lastCreated.personId ? (
                  <Link
                    ...
                    href={`/members/${lastCreated.personId}`}
                    style={{ backgroundColor: "var(--alvo-blue)" }}
                  >
                    Ver Perfil Completo
                  </Link>
      ...
      <section className="reception-workbench page-workbench">
        ...
        <div className="queue-panel">
          <div
            ...
            style={{ borderBottomColor: "var(--alvo-line)" }}
          >
          ...
          >
            {pendingCommunicationVisitors.length ? (
              pendingCommunicationVisitors.map((visitor) => (
                <div className="queue-item" key={`communication-${visitor.id}`}>
                  <div>
                    ...
                    <p style={{ color: "var(--alvo-ink-soft)" }}>
                      {visitor.source} · {visitor.phone || "Sem telefone"}
                    </p>
                  </div>
                  <button
                    ...
                    title={
                      visitor.phone
                        ...
                        : "Cadastre um telefone para disparar WhatsApp"
                    }
                  >
              ...
              <div className="empty-state compact">
                ...
                <p>
                  Todos os visitantes da fila local já receberam o
                  encaminhamento de boas-vindas.
                </p>
              </div>
            )}
          </div>
          ...
          >
            {celebrationGreetingVisitors.length ? (
              celebrationGreetingVisitors.map((visitor) => (
                <div className="queue-item" key={`greeting-${visitor.id}`}>
                  <div>
                    ...
                    <p style={{ color: "var(--alvo-ink-soft)" }}>
                      {visitor.note || "Visitante da celebração de hoje"}
                    </p>
                  </div>
                  <button
                    className="queue-action"
              ...
              <div className="empty-state compact">
                ...
                <p>
                  Todos os visitantes da fila local já foram marcados como
                  saudados.
                </p>
              </div>
            )}
      ...
      <section className="reception-live-grid">
        ...
        <article className="directory-panel">
          <div
            ...
            style={{
              borderBottomColor: "var(--alvo-line)",
              paddingBottom: "1rem",
              marginBottom: "1rem",
            }}
          >
            ...
            <span className="soft-pill">
              {visitorIntakes.length} cadastradas
            </span>
          </div>
          <div className="visitor-list">
            {visitorIntakes.length ? (
              visitorIntakes.map((intake) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p style={{ color: "var(--alvo-ink-soft)" }}>
                      {intake.source} - {intake.status || "Ativo"}
                    </p>
                    <small
                      style={{ color: "var(--alvo-ink-soft)", opacity: 0.8 }}
                    >
                      {intake.greeting ?? "Acolhimento pastoral pendente"}
                    </small>
                  </div>
                  <span
                    style={{
              ...
              <div className="empty-state" style={{ padding: "3rem 1rem" }}>
                ...
                >
                  Nenhuma Entrada Cloud Encontrada
                </strong>
                <p style={{ color: "var(--alvo-ink-soft)", opacity: 0.8 }}>
                  O simulador local está pronto. Conecte o Firebase para
                  carregar registros dinâmicos.
                </p>
              </div>
            )}
          </div>
        ...
        <article className="directory-panel">
          ...
          >
            <div>
              <p className="eyebrow" style={{ color: "#10b981" }}>
                Monitor de Funil
              </p>
              <h2 style={{ color: "var(--alvo-ink)" }}>
          ...
          <div className="visitor-list">
            {visitorJourneys.length ? (
              visitorJourneys.map((journey) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p style={{ color: "var(--alvo-ink-soft)" }}>
                      Estágio: {journey.currentStage} · Origem:{" "}
                      {journey.originChannel}
                    </p>
                    <small
                      style={{ color: "var(--alvo-ink-soft)", opacity: 0.8 }}
                    >
                      Próximo passo planejado em:{" "}
                      {journey.nextActionAt ?? "Imediato"}
                    </small>
                  </div>
                  <span
                    style={{
              ...
              <div className="empty-state" style={{ padding: "3rem 1rem" }}>
                ...
                >
                  Nenhuma Jornada Ativa Carregada
                </strong>
                <p style={{ color: "var(--alvo-ink-soft)", opacity: 0.8 }}>
                  As jornadas pastorais ajudam a consolidar os novos convertidos
                  em membros ativos.
                </p>
              </div>
            )}
