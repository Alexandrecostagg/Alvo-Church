...
export function UsersView() {
  const { firebaseConfig, organizationId, tenantReady, user, refreshRoles } =
    useAppAuth();
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const list = await sdk.fetchTenantUsers(firebaseConfig, {
        organizationId,
      });
      if (!cancelled) {
        setUsers(list);
  ...
  async function handleRoleChange(userId: string, newRole: AppRole) {
    ...
    try {
      const sdk = await import("@alvo/firebase");
      await sdk.updateTenantUserRoles(firebaseConfig, {
        organizationId,
        userId,
        roles: [newRole],
      });
  ...
  return (
    <div style={{ padding: "2rem 1.5rem", maxWidth: 760, margin: "0 auto" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        >
          Gerencie os níveis de acesso de cada usuário.
        </p>
      </div>
                        ...
                        >
                          ...
                          <span
                            style={{
                              ...
                              fontWeight: primaryRole === opt.value ? 500 : 400,
                            }}
                          >
                            {opt.label}
                          </span>
