...
export function KidsSettingsView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (settings) {
        if (settings.qrGeneratorRoles?.length)
          setRoles(new Set(settings.qrGeneratorRoles));
        setTeamIds(new Set(settings.kidsTeamIds ?? []));
      }
    } catch (e) {
      setError(
        friendlyError(e, "Erro ao carregar configuração da Segurança Kids"),
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      >
        Defina quem pode operar o check-in por QR e quais salas do ministério
        infantil existem. O app usa essas regras para liberar a entrada/saída
        das crianças.
      </p>
      ...
      <section style={card}>
        ...
        >
          Admins da igreja já são sempre autorizados.
        </p>
      ...
      <section style={card}>
        ...
        >
          Marque quais equipes de escala representam salas kids. Voluntários
          escalados nessas salas operam o check-in.
        </p>
        {teams.length === 0 ? (
          <p style={{ fontSize: 13, color: "var(--color-text-secondary)" }}>
            Nenhuma equipe de escala cadastrada ainda. Crie as salas em Escalas.
          </p>
        ) : (
          <div style={{ display: "grid", gap: 8 }}>
            {teams.map((t) => (
              <label key={t.id} style={checkRow}>
                <input
                  ...
                  onChange={() => setTeamIds((s) => toggle(s, t.id))}
                />
                <span>
                  {t.name}{" "}
                  <span
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 10,
          border: "none",
          background: saved ? "#10b981" : "#534AB7",
          color: "#fff",
          fontSize: 14,
          fontWeight: 700,
          cursor: "pointer",
