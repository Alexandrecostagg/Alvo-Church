...
import {
  useOrgFeatures,
  type GroupsModelType,
  type OrgTier,
} from "../../../contexts/OrgFeaturesContext";
import {
...
const GROUPS_MODEL_OPTIONS: {
  value: GroupsModelType;
  label: string;
  desc: string;
}[] = [
  {
    value: "cell",
    label: "Células",
    desc: "Grupos geográficos semanais — modelo de célula clássico",
  },
  {
    value: "gc",
    label: "G.C.",
    desc: "Grupos de Crescimento — mesmo modelo, nome diferente",
  },
  {
    value: "leadership",
    label: "Lideranças",
    desc: "Liderança de Casais, Jovens, Famílias — grupos temáticos",
  },
  {
    value: "generic",
    label: "Grupos",
    desc: "Nomenclatura genérica sem vínculo a modelo específico",
  },
];
...
const ORG_TIER_OPTIONS: {
...
}[] = [
  {
    value: "solo",
    label: "Igreja Solo",
    desc: "Igreja independente — simples, sem hierarquia",
    icon: Building2,
  },
  {
    value: "campus",
    label: "Multi-campus",
    desc: "Uma sede com múltiplos campi sob a mesma liderança",
    icon: Layers,
  },
  {
    value: "network",
    label: "Rede",
    desc: "Instituição com igrejas afiliadas — visão consolidada",
    icon: Layers,
  },
  {
    value: "denomination",
    label: "Denominação",
    desc: "Estrutura formal com hierarquia nacional e múltiplos níveis",
    icon: Layers,
  },
];
...
const MODULE_DEFS: ModuleDef[] = [
  {
    key: "visitors",
    label: "Recepção & Visitantes",
    desc: "Formulários de visita, painel do pastor, triagem de novos contatos",
    icon: Users,
    color: "#3b82f6",
  },
  {
    key: "groups",
    label: "Células",
    desc: "Grupos de discipulado, reuniões, chamadas e relatórios de célula",
    icon: Waypoints,
    color: "#10b981",
  },
  {
    key: "tribes",
    label: "Tribos Ministeriais",
    desc: "Classificação vocacional por IA, perfil ministerial e indicação",
    icon: Tent,
    color: "#f97316",
  },
  {
    key: "journeys",
    label: "Jornadas & EAD",
    desc: "Trilha de integração de novos membros e escola de discipulado EAD",
    icon: MapIcon,
    color: "#8b5cf6",
  },
  {
    key: "events",
    label: "Eventos",
    desc: "Agenda estratégica, inscrições, check-in e relatórios de presença",
    icon: CalendarRange,
    color: "#06b6d4",
  },
  {
    key: "volunteers",
    label: "Escalas & Louvor",
    desc: "Escalas de serviço, gestão de equipes e cifras para músicos",
    icon: Handshake,
    color: "#ec4899",
  },
  {
    key: "communication",
    label: "Comunicação",
    desc: "Envio de mensagens, push, WhatsApp e segmentação por grupo",
    icon: MessageSquareText,
    color: "#64748b",
  },
  {
    key: "giving",
    label: "Doações & Dízimos",
    desc: "Recebimento de dízimos e ofertas via PIX, cartão e boleto",
    icon: HeartHandshake,
    color: "#16a34a",
  },
  {
    key: "finance",
    label: "Finanças",
    desc: "Relatórios financeiros, receitas, despesas e fluxo de caixa",
    icon: Landmark,
    color: "#d97706",
  },
  {
    ...
    desc: "Análise preditiva de risco de desengajamento e insights por membro",
    icon: Bot,
    color: "#7c3aed",
  },
  {
    key: "marketplace",
    label: "Marketplace",
    desc: "Comunidade de trocas e serviços entre membros da igreja",
    icon: Store,
    color: "#0ea5e9",
  },
  {
    ...
    desc: "Check-in de crianças, identificação e rastreio de retirada",
    icon: ShieldCheck,
    color: "#f43f5e",
  },
  {
    ...
    desc: "QR Codes para visitantes e links de cadastro sem login",
    icon: GraduationCap,
    color: "#84cc16",
  },
];
...
export function SettingsView() {
  ...
  const [pixKey, setPixKey] = useState(
    tenantRuntime?.settings?.branding?.pixKey ?? "",
  );
  const [pixName, setPixName] = useState(
    tenantRuntime?.settings?.branding?.pixReceiverName ?? "",
  );
  const [pixWhatsapp, setPixWhatsapp] = useState(
    tenantRuntime?.settings?.branding?.givingWhatsappNumber ?? "",
  );
  ...
  const [groupsModel, setGroupsModel] = useState<GroupsModelType>(
    (tenantRuntime?.settings?.branding?.groupsModelType as
      | GroupsModelType
      | undefined) ?? "cell",
  );
  ...
  async function saveModules() {
    ...
    try {
      ...
      for (const def of MODULE_DEFS) {
        (modules as Record<string, unknown>)[def.key] = {
          enabled: moduleState[def.key],
        };
      }
  ...
  async function savePixConfig() {
    ...
    try {
      ...
      setPixBaseline({
        key: updated.pixKey,
        name: updated.pixReceiverName,
        whatsapp: updated.givingWhatsappNumber,
      });
      setPixSaved(true);
      ...
      <header className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">
            Gerencie módulos ativos, links públicos e personalizações da
            organização
          </p>
        </div>
      </header>
      ...
      <section className="content-section">
        <div className="section-header">
          <div>
            ...
            >
              {enabledCount} de {MODULE_DEFS.length} módulos ativos — controle o
              que aparece no menu de navegação
            </p>
          </div>
          <button
            onClick={saveModules}
        ...
        {!isFirebaseWebRuntimeConfigured(firebaseConfig) && (
          <div className="settings-demo-banner">
            ...
            <span>
              Modo demonstração — as alterações de módulos não serão salvas
            </span>
          </div>
        )}
          ...
          {MODULE_DEFS.map((def) => {
            ...
            return (
              <div
                ...
                style={{
                  borderColor: enabled ? `${def.color}30` : "var(--alvo-line)",
              ...
              >
                ...
                <Toggle
                  enabled={enabled}
                  onChange={(v) => toggleModule(def.key, v)}
                />
              </div>
            );
          })}
      ...
      <section className="content-section">
        <div className="section-header">
          <div>
            ...
            >
              Define como o módulo de grupos aparece no menu e nas telas da sua
              instituição
            </p>
          </div>
          <button
            ...
            disabled={
              groupsSaving || !isFirebaseWebRuntimeConfigured(firebaseConfig)
            }
            className="btn-primary btn-sm"
            style={{
              display: "flex",
        ...
        <div style={{ display: "grid", gap: 16, maxWidth: 640 }}>
          ...
          >
            {GROUPS_MODEL_OPTIONS.map((opt) => (
              <button
                key={opt.value}
                onClick={() => {
                  setGroupsModel(opt.value);
                  setGroupsCustomLabel("");
          ...
          <div style={{ display: "grid", gap: 6 }}>
            ...
            <input
              ...
              onChange={(e) => {
                setGroupsCustomLabel(e.target.value);
                setGroupsSaved(false);
              }}
              style={{
                padding: "10px 14px",
      ...
      <section className="content-section">
        <div className="section-header">
          <div>
            ...
            >
              Define a estrutura institucional e desbloqueia funcionalidades
              específicas por porte
            </p>
          </div>
          <button
        ...
        {(selectedTier === "network" || selectedTier === "denomination") && (
          <div className="settings-demo-banner" style={{ marginTop: 12 }}>
            ...
            <span>
              ...
              será habilitado no menu lateral — gerencie igrejas afiliadas e
        ...
        <div className="section-header">
          ...
          <button
            ...
            disabled={
              pixSaving ||
              !isFirebaseWebRuntimeConfigured(firebaseConfig) ||
              (pixConfigured && !pixDirty && !pixSaved)
            }
            className="btn-primary btn-sm"
            style={{
              display: "flex",
              ...
              ...(pixSaved || (pixConfigured && !pixDirty)
                ? { background: "var(--alvo-green)", opacity: 1 }
                : {}),
            }}
          >
            {pixSaving ? (
        ...
        <div style={{ display: "grid", gap: 12, maxWidth: 500 }}>
          ...
          <div style={{ display: "grid", gap: 6 }}>
            ...
            >
              ...
              >
                (botão "Falar no WhatsApp" na página pública de doação)
              </span>
            </label>
            <input
              type="tel"
