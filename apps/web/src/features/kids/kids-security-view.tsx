...
export function KidsSecurityView() {
  ...
  const load = useCallback(async () => {
    if (!configured || !firebaseReady || !organizationId || !user || !isFirebaseWebRuntimeConfigured(firebaseConfig)) {
      setLoading(false);
      return;
    }
    try {
  ...
  return (
    <main className="member-app-shell">
      ...
      <section className="kids-status-container">
        <p className="section-subtitle">O crachá com QR de retirada dos seus filhos com check-in ativo.</p>

        {loading ? (
          <p style={{ color: "var(--alvo-ink-soft)", fontSize: "0.9rem", padding: "1.5rem 0" }}>Carregando…</p>
        ) : myCheckIns.length === 0 ? (
          <div style={{ textAlign: "center", padding: "2.5rem 1rem", color: "var(--alvo-ink-soft)" }}>
            <Baby size={40} style={{ opacity: 0.3, marginBottom: 10 }} />
            <p style={{ fontSize: "0.9rem", fontWeight: 500 }}>Nenhum check-in ativo.</p>
            <p style={{ fontSize: "0.78rem", marginTop: 4 }}>O líder da sala Kids faz o check-in do seu filho na entrada — o crachá aparece aqui.</p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem", marginTop: "1rem" }}>
            {myCheckIns.map((c) => (
              <div key={c.id} className="token-card" style={{ padding: "1.25rem", border: "1px solid var(--alvo-line)", borderRadius: 20 }}>
                <div className="token-header" style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                ...
                <div style={{ textAlign: "center", marginTop: 10 }}>
                  ...
                  {c.checkedInAt && (
                    <p style={{ fontSize: "0.72rem", color: "var(--alvo-ink-soft)", margin: "4px 0 0", display: "inline-flex", alignItems: "center", gap: 4 }}>
                      <Clock size={12} /> Entrada às {new Date(c.checkedInAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
                    </p>
                  )}
                </div>
              </div>
      ...
      <footer className="kids-history">
        <button className="ghost-button full" onClick={() => { setLoading(true); void load(); }} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          <History size={18} />
          Atualizar
        </button>
      </footer>
