...
describe("checkScheduleConflict", () => {
  const baseAssignment: Omit<
    ServiceAssignment,
    "id" | "personId" | "serviceDate" | "status"
  > = {
  ...
  it("should return conflict when personId and serviceDate match, and status is not declined", () => {
    const conflictAssignment = createAssignment(
      "existing-1",
      "person-1",
      "2024-01-01T10:00:00Z",
      "confirmed",
    ...
    expect(checkScheduleConflict(assignments, newAssignment)).toEqual(
      conflictAssignment,
    );
  });
  ...
  it("should return the first conflict if multiple conflicts exist", () => {
    const conflictAssignment1 = createAssignment(
      "existing-1",
      "person-1",
      "2024-01-01T10:00:00Z",
      "confirmed",
    );
    const conflictAssignment2 = createAssignment(
      "existing-2",
      "person-1",
      "2024-01-01T10:00:00Z",
      "pending",
  ...
  it("should find conflict among non-conflicting assignments", () => {
    const otherAssignment1 = createAssignment(
      "existing-1",
      "person-2",
      "2024-01-01T10:00:00Z",
      "confirmed",
    );
    const otherAssignment2 = createAssignment(
      "existing-2",
      "person-1",
      "2024-01-02T10:00:00Z",
      "confirmed",
