...
describe('generatePixPayload', () => {
  ...
  it('should generate valid CRC16', () => {
    ...
    // Manually calculate CRC for the generated payload to verify consistency
    // Re-implement CRC16 here for testing purposes
    let calcCrc = 0xffff;
    for (let i = 0; i < dataWithoutCrc.length; i++) {
      calcCrc ^= dataWithoutCrc.charCodeAt(i) << 8;
      for (let j = 0; j < 8; j++) {
        calcCrc = calcCrc & 0x8000 ? (calcCrc << 1) ^ 0x1021 : calcCrc << 1;
      }
      calcCrc &= 0xffff;
    }
    const expectedCrc = calcCrc.toString(16).toUpperCase().padStart(4, "0");
