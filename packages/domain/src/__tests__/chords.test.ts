...
describe("transposeChordsText", () => {
  ...
  it("should transpose a simple chord", () => {
    ...
    expect(transposeChordsText(lyrics, "C", "D")).toBe("A [D] song in [G] and [A]");
  });

  it("should transpose minor chords and suffixes", () => {
    const lyrics = "[Am] Amazing [Dm7] Grace [Em7]";
    expect(transposeChordsText(lyrics, "C", "D")).toBe("[Bm] Amazing [Em7] Grace [F#m7]");
  });

  it("should transpose flat keys correctly", () => {
    const lyrics = "[F] Something [Bb] else [C7]";
    expect(transposeChordsText(lyrics, "F", "G")).toBe("[G] Something [C] else [D7]");
  });
  ...
  it("should leave unmatched brackets unchanged", () => {
    ...
    expect(transposeChordsText(lyrics, "C", "D")).toBe("[D] This is [just a note] not a chord [G]");
  });
});
